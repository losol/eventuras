...
public class ConvertoPdfRenderServiceTest : IDisposable
{
        ...
        string apitoken = null)
    {
        ...
        var convertoClient = new ConvertoClient(_httpClientFactory, options, loggerFactory.CreateLogger<ConvertoClient>());

        return new ConvertoPdfRenderService(convertoClient, options, loggerFactory.CreateLogger<ConvertoPdfRenderService>());
    }
    ...
    {
        await using var stream = await NewService("/convert/html/to/pdf2").RenderHtmlAsync("<html></html>", new PdfRenderOptions());
        await CheckEmptyAsync(stream);
    }
    ...
    {
        await using var stream = await NewService(apitoken: "invalid").RenderHtmlAsync("<html></html>", new PdfRenderOptions());
        await CheckEmptyAsync(stream);
    }
