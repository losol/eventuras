...
public static class JTokenExtensions
{
    ...
    public static void CheckRegistration(this JToken token, Registration registration, bool checkUserInfo = false, bool checkEventInfo = false)
    {
        ...
        if (checkUserInfo) token.Value<JToken>("user").CheckUser(registration.User);

        if (checkEventInfo) token.Value<JToken>("event").CheckEvent(registration.EventInfo);
    }
    ...
    public static void CheckCertificate(this JToken token, Certificate certificate)
    {
        ...
        Assert.Equal(certificate.IssuingOrganization?.LogoBase64, token.Value<string>("issuerOrganizationLogoBase64"));
        Assert.Equal(certificate.IssuedByName, token.Value<string>("issuerPersonName"));
        Assert.Equal(certificate.IssuingUser?.SignatureImageBase64, token.Value<string>("issuerPersonSignatureImageBase64"));
    }
    ...
    public static JToken CheckOrder(
        this JToken token,
        Order order,
        ...
        bool checkItems = false)
    {
        ...
        else if (checkUserNull) { Assert.Empty(userToken); }
        ...
        else if (checkRegistrationNull) { Assert.Empty(registrationToken); }

        if (checkItems)
        {
            var itemsToken = token.Value<JArray>("items");
            Assert.NotNull(itemsToken);
            itemsToken.CheckArray((t, item) => t.CheckOrderItem(item), order.OrderLines?.ToArray() ?? Array.Empty<OrderLine>());
        }
    ...
    {
        if (item.ProductId.HasValue) token.Value<JToken>("product").CheckProduct(item.Product);

        if (item.ProductVariantId.HasValue) token.Value<JToken>("productVariant").CheckProductVariant(item.ProductVariant);
    ...
    {
        Assert.Equal(order.OrderId, token.Value<int>("orderId"));
        token.Value<JArray>("items").CheckArray((t, item) => t.CheckOrderItem(item), order.OrderLines?.ToArray() ?? Array.Empty<OrderLine>());
    }
    ...
    public static void CheckProduct(this JToken token, Product product, params ProductVariant[] variants)
    {
        ...
        if (variants.Any()) token.Value<JArray>("variants").CheckArray((t, v) => t.CheckProductVariant(v), variants);
    }
        ...
        int? totalRecipients = null)
    {
        ...
        Assert.Equal(notification.Created.ToString("yyyy-MM-ddTHH:mm:ss"), token.Value<DateTime>("created").ToString("yyyy-MM-ddTHH:mm:ss"));
        ...
        if (totalSent.HasValue || totalErrors.HasValue || totalRecipients.HasValue)
        {
            ...
            if (totalSent.HasValue) Assert.Equal(totalSent, stats.Value<int>("sent"));

            if (totalErrors.HasValue) Assert.Equal(totalErrors, stats.Value<int>("errors"));

            if (totalRecipients.HasValue) Assert.Equal(totalRecipients, stats.Value<int>("recipients"));
        }
    }
    ...
    public static void CheckNotificationRecipient(this JToken token, NotificationRecipient recipient)
    {
        ...
        Assert.Equal(recipient.Created.ToString("yyyy-MM-ddTHH:mm:ss"), token.Value<DateTime>("created").ToString("yyyy-MM-ddTHH:mm:ss"));
        Assert.Equal(recipient.Sent?.ToString("yyyy-MM-ddTHH:mm:ss"), token.Value<DateTime?>("sent")?.ToString("yyyy-MM-ddTHH:mm:ss"));
    ...
    public static JArray CheckArray<T>(this JArray token, Action<JToken, T> f, params T[] values)
    {
        ...
        for (var i = 0; i < token.Count; ++i) f(token[i], values[i]);
    ...
    public static JToken CheckPaging<T>(this JToken token, Action<JToken, T> f, params T[] data) => CheckPaging(token, 1, null, null, f, data);

    public static JToken CheckPaging<T>(this JToken token, int page, int? total, Action<JToken, T> f, params T[] data)
    ...
    public static JToken CheckPaging<T>(this JToken token, int page, int? count, int? total, Action<JToken, T> f, params T[] data)
    {
        ...
        if (count.HasValue) Assert.Equal(count.Value, token.Value<int>("count"));

        if (total.HasValue) Assert.Equal(total.Value, token.Value<int>("total"));
    ...
    private static DateTimeOffset? ReadAsDateTimeOffset(this JToken t, string path)
    {
