...
public class EventProductsControllerTests : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    ...
    public async Task List_Should_Not_Return_Archived_Product_Variants()
    {
        ...
        var arr = await response.AsArrayAsync();
        arr.CheckArray((t, p) => t.CheckProduct(p, v2.Entity, v4.Entity), product.Entity);
    }
    ...
    public async Task Add_Should_Check_Admin_Org()
    {
        ...
        var response = await client.PostAsync($"/v3/events/{evt.Entity.EventInfoId}/products?orgId={org1.Entity.OrganizationId}",
            new { name = "test" });
        response.CheckForbidden();
    }
    ...
    public async Task Add_Should_Be_Available_For_Power_Admin(string role)
    {
        ...
        var product = await scope.Db.Products.SingleOrDefaultAsync(p => p.EventInfoId == evt.Entity.EventInfoId);
    ...
    public async Task Add_Should_Allow_Org_Admin_To_Add_New_Product_With_Max_Data()
    {
        ...
        var response = await client.PostAsync($"/v3/events/{evt.Entity.EventInfoId}/products?orgId={org.Entity.OrganizationId}",
            new
            {
                name = "test",
                description = "desc",
    ...
    {
        var response = await _factory.CreateClient().PutAsync("/v3/events/1/products/1001");
        response.CheckUnauthorized();
    }
    ...
    {
        var response = await _factory.CreateClient().Authenticated().PutAsync("/v3/events/1/products/1001");
        response.CheckForbidden();
    }
    ...
    public async Task Update_Should_Return_Not_Found_For_Unknown_Event_Id()
    {
        ...
        var response = await client.PutAsync("/v3/events/11111/products/1001",
            new
            {
                published = true,
                visibility = "event",
    ...
    public async Task Update_Should_Allow_Org_Admin_To_Change_Product_Published_Status_And_Visibility()
    {
        ...
        using var m = await scope.CreateOrganizationMemberAsync(admin.Entity, org.Entity, role: Roles.Admin);
        using var evt = await scope.CreateEventAsync(organization: org.Entity);
        using var p = await scope.CreateProductAsync(evt.Entity, published: false, visibility: ProductVisibility.Event);

        var client = _factory.CreateClient().AuthenticatedAs(admin.Entity, Roles.Admin);

        var response = await client.PutAsync($"/v3/events/{evt.Entity.EventInfoId}/products/{p.Entity.ProductId}?orgId={org.Entity.OrganizationId}",
            new { visibility = "collection", published = true });

        var token = await response.CheckOk().AsTokenAsync();

        var updatedProduct = await scope.Db.Products.AsNoTracking().SingleAsync(product => product.ProductId == p.Entity.ProductId);
    ...
    public async Task Update_Should_Not_Allow_Other_Org_Admin_To_Update_Product()
    {
        ...
        var response = await client.PutAsync($"/v3/events/{evt.Entity.EventInfoId}/products/{p.Entity.ProductId}?orgId={org1.Entity.OrganizationId}",
    ...
    public async Task Update_Should_Allow_Power_Admin_To_Change_Product_Published_Status_And_Visibility(string role)
    {
        ...
        var response = await client.PutAsync($"/v3/events/{evt.Entity.EventInfoId}/products/{p.Entity.ProductId}",
            new { visibility = "collection", published = true });
    ...
    public async Task Should_Return_Ok_For_Archived_Product()
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAsSystemAdmin();
        var response = await client.DeleteAsync($"/v3/events/{evt.Entity.EventInfoId}/products/{product.Entity.ProductId}");
        response.CheckOk();
    }
    ...
    public async Task Should_Allow_Org_Admin_To_Archive_Product()
    {
        ...
        var updatedProduct = await scope.Db.Products.AsNoTracking().SingleOrDefaultAsync(p => p.ProductId == product.Entity.ProductId);
