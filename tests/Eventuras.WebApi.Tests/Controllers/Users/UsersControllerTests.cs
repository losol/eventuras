...
public class UsersControllerTests : IClassFixture<CustomWebApiApplicationFactory<Startup>>, IDisposable
{
    ...
    public async Task List_Users_Should_Return_Empty_List()
    {
        ...
        var client = _factory.CreateClient().Authenticated(role: Roles.SuperAdmin);
    ...
    public async Task List_Users_Should_Use_Paging(string role)
    {
        ...
        using var user3 = await scope.CreateUserAsync("Test Person 3");
        using var user2 = await scope.CreateUserAsync("Test Person 2");
        using var user1 = await scope.CreateUserAsync("Test Person 1");

        var response = await client.GetAsync("/v3/users?page=1&count=2");
        response.CheckOk();

        var json = await response.AsTokenAsync();
        json.CheckPaging(1, 2, 3, (token, u) => token.CheckUser(u), user1.Entity, user2.Entity);

        response = await client.GetAsync("/v3/users?page=2&count=2");
        response.CheckOk();

        json = await response.AsTokenAsync();
        json.CheckPaging(2, 2, 3, (token, u) => token.CheckUser(u), user3.Entity);
    }
    ...
    public async Task List_Should_Filter_By_Query_Param()
    {
        ...
        using var user5 = await scope.CreateUserAsync("Test Person 5", "other@email.com", phone: null); // no phone
        using var user4 = await scope.CreateUserAsync("Test Person 4", "testperson4@email.com", phone: "+1234567890");
        using var user3 = await scope.CreateUserAsync("Test Person 3", "testperson3@email.com", phone: "+11122223333444");
        using var user2 = await scope.CreateUserAsync("Test Person 2", "testperson2@email.com", phone: "+2222222221");
        using var user1 = await scope.CreateUserAsync("Test Person 1", "testperson1@email.com", phone: "+11111111111");
    ...
    private static async Task CheckListAsync(HttpClient client, object queryParams, params IDisposableEntity<ApplicationUser>[] users)
    {
    ...
    public async Task List_Should_Use_Order_Param()
    {
        ...
        using var user5 = await scope.CreateUserAsync("Test Person 5", "other@email.com", phone: null); // no phone
        using var user4 = await scope.CreateUserAsync("Test Person 4", "testperson1@email.com", phone: "+1234567890");
        using var user3 = await scope.CreateUserAsync("Test Person 3", "testperson2@email.com", phone: "+11122223333444");
        using var user2 = await scope.CreateUserAsync("Test Person 2", "testperson3@email.com", phone: "+2222222221");
        using var user1 = await scope.CreateUserAsync("Test Person 1", "testperson4@email.com", phone: "+11111111111");
    ...
    public async Task Update_User_Should_Not_Return_Conflict_If_Existing_Email_Is_Archived()
    {
        ...
        var updateUser = await scope.Db.Users.AsNoTracking().SingleAsync(u => u.Id == user.Entity.Id);
    ...
    public async Task Should_Update_User_With_Max_Data()
    {
        ...
        var updatedUser = await scope.Db.Users.AsNoTracking().SingleAsync(u => u.Id == user.Entity.Id);
