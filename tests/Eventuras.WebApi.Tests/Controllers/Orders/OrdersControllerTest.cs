...
public class OrdersControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>, IDisposable
{
    ...
    public async Task List_Should_Use_Sorting_And_Paging()
    {
        ...
        using var o1 = await scope.CreateOrderAsync(r.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(1)));
        using var o2 = await scope.CreateOrderAsync(r.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(2)));
        using var o3 = await scope.CreateOrderAsync(r.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(3)));
    ...
    public async Task List_Should_Return_Full_Info()
    {
        ...
        using var order = await scope.CreateOrderAsync(r.Entity, user: user.Entity, product: p.Entity, variant: v.Entity);

        var client = _factory.CreateClient().AuthenticatedAsSuperAdmin();
        var response = await client.GetAsync("/v3/orders?includeUser=true&includeRegistration=true");
        var json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, o) => t.CheckOrder(o, true, checkRegistration: true, checkItems: true), order.Entity);
    }
    ...
    {
        var updatedOrder = await scope.Db.Orders.AsNoTracking().SingleAsync(o => o.OrderId == order.Entity.OrderId);
    ...
    public async Task Should_Allow_To_Cancel_Order_For_Org_Admin()
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAs(admin.Entity, Roles.Admin);
        var response = await client.DeleteAsync($"/v3/orders/{order.Entity.OrderId}?orgId={org.Entity.OrganizationId}");
        response.CheckOk();
    ...
    public async Task Should_Return_Order_Related_To_Admin_Org()
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAs(admin.Entity, Roles.Admin);
        var response = await client.GetAsync($"/v3/orders/{order.Entity.OrderId}?orgId={org.Entity.OrganizationId}");
        var json = await response.CheckOk().AsTokenAsync();
        json.CheckOrder(order.Entity);
    }
    ...
    public async Task Should_Not_Return_Order_Not_Related_To_Admin_Org()
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAs(admin.Entity, Roles.Admin);
        var response = await client.GetAsync($"/v3/orders/{order.Entity.OrderId}?orgId={otherOrg.Entity.OrganizationId}");
        response.CheckForbidden();
    }
    ...
    public async Task Get_Should_Return_Max_Data()
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAsSystemAdmin();
        var response = await client.GetAsync($"/v3/orders/{order.Entity.OrderId}?includeUser=true&includeRegistration=true");
        var json = await response.CheckOk().AsTokenAsync();
    ...
    {
        var response = await _factory.CreateClient().PostAsync("/v3/orders");
    ...
    {
        var response = await _factory.CreateClient().AuthenticatedAsSystemAdmin().PostAsync("/v3/orders");
    ...
    public async Task Should_Allow_Regular_User_To_Create_Order()
    {
        ...
        var response = await _factory.CreateClient().AuthenticatedAs(user.Entity).PostAsync("/v3/orders", GetOrderCreateData(reg, p, v, p2));
        ...
        var lines = createdOrder.OrderLines.ToDictionary(l => l.ProductId);
    ...
    public async Task Should_Not_Allow_Regular_User_To_Create_Order_For_Someone_Else()
    {
        ...
        var response = await _factory.CreateClient().AuthenticatedAs(u1.Entity).PostAsync("/v3/orders", GetOrderCreateData(reg, p, v, p2));
    ...
    public async Task Should_Allow_To_Create_Order_For_Admin()
    {
        ...
        var response = await _factory.CreateClient()
            ...
            .PostAsync($"/v3/orders?orgId={org.Entity.OrganizationId}", GetOrderCreateData(reg, p, v, p2));
    ...
    public async Task Should_Allow_Power_Admin_To_Create_Any_Order(string role)
    {
        ...
        var response = await _factory.CreateClient().Authenticated(role: role).PostAsync("/v3/orders", GetOrderCreateData(reg, p, v, p2));
    ...
    {
        var response = await _factory.CreateClient().PutAsync("/v3/orders/1");
    ...
    public async Task Should_Allow_Regular_User_To_Update_Own_Order()
    {
        ...
        var response = await _factory.CreateClient()
            ...
            .PutAsync($"/v3/orders/{order.Entity.OrderId}", GetOrderUpdateData(p, v, p2));
    ...
    public async Task Should_Allow_To_Update_Order_For_Admin()
    {
        ...
        var response = await _factory.CreateClient()
            .AuthenticatedAs(admin.Entity, Roles.Admin)
            .PutAsync($"/v3/orders/{order.Entity.OrderId}?orgId={org.Entity.OrganizationId}", GetOrderUpdateData(p, v, p2));
    ...
    public async Task Should_Allow_To_Update_Order_In_Status(Order.OrderStatus status)
    {
        ...
        using var p2 = await scope.CreateProductAsync(evt.Entity);
        using var v = await scope.CreateProductVariantAsync(p.Entity);
        using var order = await scope.CreateOrderAsync(reg.Entity, p.Entity, v.Entity, status: status);
    ...
    {
        var createdOrder = await scope.Db.Orders.AsNoTracking().Include(o => o.OrderLines).SingleAsync();
    ...
    private static async Task<Order> CheckOrderUpdatedAsync(
        ...
        IDisposableEntity<Product> p2)
    {
        var updatedOrder = await scope.Db.Orders.AsNoTracking()
    ...
    private static void CheckOrderAsync(Order order, IDisposableEntity<Product> p, IDisposableEntity<ProductVariant> v, IDisposableEntity<Product> p2)
    {
    ...
    private static object GetOrderUpdateData(IDisposableEntity<Product> p, IDisposableEntity<ProductVariant> v, IDisposableEntity<Product> p2)
        => new
        {
            lines = GetOrderLinesData(p, v, p2),
        };

    private static object GetOrderLinesData(IDisposableEntity<Product> p, IDisposableEntity<ProductVariant> v, IDisposableEntity<Product> p2)
    {
        return new[]
        {
            new
