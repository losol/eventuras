...
public class RegistrationCertificateControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    ...
    {
        var response = await _factory.CreateClient().PostAsync("/v3/registrations/1001/certificate/send");
    ...
    {
        var response = await _factory.CreateClient().AuthenticatedAsSuperAdmin().PostAsync("/v3/registrations/1001/certificate/send");
    ...
    public async Task Should_Not_Allow_Other_Org_Admin_To_Send_Certificate()
    {
        ...
        using var mem = await scope.CreateOrganizationMemberAsync(otherOrgAdmin.Entity, otherOrg.Entity, role: Roles.Admin);
        using var evt = await scope.CreateEventAsync(organization: org.Entity);
        using var user = await scope.CreateUserAsync();
        using var reg = await scope.CreateRegistrationAsync(evt.Entity, user.Entity);
        ...
        var response = await _factory.CreateClient()
            ...
            .PostAsync($"/v3/registrations/{reg.Entity.RegistrationId}/certificate/send?orgId={org.Entity.OrganizationId}");
    ...
    private EmailExpectation SetupEmailExpectation(IDisposableEntity<ApplicationUser> user, IDisposableEntity<EventInfo> evt)
        => _factory.EmailSenderMock.ExpectEmail()
