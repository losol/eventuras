...
public class NotificationRecipientsControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    private readonly CustomWebApiApplicationFactory<Startup> _factory;

    public NotificationRecipientsControllerTest(CustomWebApiApplicationFactory<Startup> factory)
    {
    ...
    public async Task List_Should_Check_Query_Params(object queryParams)
    {
        ...
        var client = _factory.CreateClient().AuthenticatedAsSystemAdmin();
        var response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients", queryParams);
    ...
    public async Task Should_Limit_Recipients_For_Regular_User()
    {
        ...
        using var notification = await scope.CreateEmailNotificationAsync(recipientUsers: new[] { u1.Entity, u2.Entity });

        var client = _factory.CreateClient().AuthenticatedAs(u1.Entity);

        var response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients");

        var json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), notification.Entity.Recipients.First());
    }
    ...
    public async Task Should_List_Recipients_For_Org_Admin()
    {
        ...
        using var notification = await scope.CreateEmailNotificationAsync(organization: org.Entity, recipientUsers: new[] { user.Entity });
    ...
    public async Task Should_Not_Limit_Recipients_For_Power_Admin(string role)
    {
        ...
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r),
            notification.Entity.Recipients.Reverse().ToArray()); // last goes first (default order=created, desc)
    }
    ...
    public async Task List_Should_Use_Paging_Query_Params()
    {
        ...
        using var notification = await scope.CreateEmailNotificationAsync(recipientUsers: new[] { u1.Entity, u2.Entity, u3.Entity });

        var recipients = notification.Entity.Recipients.ToArray();

        var client = _factory.CreateClient().AuthenticatedAsSuperAdmin();
        var response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?page=1&count=2");

        var json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging(1, 3, (t, r) => t.CheckNotificationRecipient(r), recipients[2], recipients[1]);

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?page=2&count=2");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging(2, 3, (t, r) => t.CheckNotificationRecipient(r), recipients[0]);
    }
    ...
    public async Task List_Should_Use_Order_Query_Params()
    {
        ...
        var json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.Reverse().ToArray()); // default sort by create date, desc

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=created");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.Reverse().ToArray()); // still the same

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=created&desc=true");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.Reverse().ToArray()); // still the same

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=created&desc=false");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.ToArray());

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=sent");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.ToArray());

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=sent&desc=true");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.ToArray());

        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?order=sent&desc=false");

        json = await response.CheckOk().AsTokenAsync();
        json.CheckPaging((t, r) => t.CheckNotificationRecipient(r), recipients.Reverse().ToArray());
    }
    ...
    public async Task List_Should_Use_Query_Param()
    {
        ...
        response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients",
            new
            {
                query = "SECOND",
            });
    ...
    public async Task List_Should_Use_Sent_Param()
    {
        ...
        var response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?sentOnly=true");
    ...
    public async Task List_Should_Use_ErrorsOnly_Param()
    {
        ...
        var response = await client.GetAsync($"/v3/notifications/{notification.Entity.NotificationId}/recipients?errorsOnly=true");
