...
public class EmailNotificationsControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>, IDisposable
{
    ...
    public async Task Should_Not_Send_Email_Notifications_To_Inaccessible_Events()
    {
        ...
        using var m1 = await scope.CreateOrganizationMemberAsync(admin.Entity, org.Entity, role: Roles.Admin);
        using var m2 = await scope.CreateOrganizationMemberAsync(otherAdmin.Entity, otherOrg.Entity, role: Roles.Admin);
        ...
        var response = await client.PostAsync($"/v3/notifications/email?orgId={org.Entity.OrganizationId}",
            new
            {
                subject = "Test 1",
        ...
        response = await client.PostAsync($"/v3/notifications/email?orgId={org.Entity.OrganizationId}",
            new
            {
                subject = "Test 2",
        ...
        await response.CheckNotificationResponse(scope, user.Entity, otherUser.Entity);

        CheckEmailSentTo("Test 2", "Test email 2", user.Entity, otherUser.Entity); // should send to both users
    ...
    /// <summary> 100 is a page size, need to ensure it sends out messages to more than 1 page of users. </summary>
    [Fact]
    public async Task Should_Send_Email_Notifications_To_More_Than_100_Users()
    {
        ...
        var users = await Task.WhenAll(Enumerable.Range(0, 101).Select(_ => scope.CreateUserAsync()));

        try
        {
            var registrations = await Task.WhenAll(users.Select(u => scope.CreateRegistrationAsync(e.Entity, u.Entity)));
            ...
            try
            {
                ...
                await response.CheckNotificationResponse(scope, users.Select(u => u.Entity).ToArray());

                _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.IsAny<EmailModel>()), Times.Exactly(users.Length));
            }
            finally
            {
    ...
    public async Task Should_Send_Email_Notifications_To_All_Users_For_Power_Admin(string role)
    {
        ...
        CheckEmailSentTo("Test 1", "Test email 1", u1.Entity, u2.Entity); // both users registered to event 1

        _factory.EmailSenderMock.Reset();

        response = await client.PostAsync("/v3/notifications/email",
            new
            {
                subject = "Test 2",
                bodyMarkdown = "Test email 2",
        ...
        CheckEmailSentTo("Test 2", "Test email 2", u2.Entity); // only user 2 is registered to event 2
    ...
    public async Task Should_Send_Email_Notifications_To_The_Given_Recipients_For_Admin(string role)
    {
        ...
        _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is(MatchUser("Test Person", "test@email.com", "Test", "Test email"))), Times.Once);

        _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is(MatchUser("Other Person", "other@email.com", "Test", "Test email"))), Times.Once);
    }
    ...
    public async Task Email_Notifications_Should_Use_Status_And_Type_Filter(string role)
    {
        ...
        await response.CheckNotificationResponse(scope, u1.Entity, u2.Entity, u3.Entity);

        CheckEmailSentTo("Letter to everyone", "Letter to everyone body", u1.Entity, u2.Entity, u3.Entity);
    }
    ...
    public async Task Should_Create_Email_Notifications_In_Db_With_No_Event_Binding()
    {
        ...
        var notification = await scope.Db.Notifications.Include(n => n.Recipients).Include(n => n.Statistics).SingleAsync();
    ...
    public async Task Should_Set_Error_For_Email_Recipient_In_Case_Of_A_Delivery_Failure()
    {
        ...
        emailSenderMock.Setup(s => s.SendEmailAsync(It.IsAny<EmailModel>())).ThrowsAsync(new Exception("test error"));
    ...
    private void CheckEmailSentTo(string subject, string body, params ApplicationUser[] users)
    {
        foreach (var u in users) // should send to both users
            _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is(MatchUser(u, subject, body))),
                Times.Once,
                $"Should've sent message {subject} to {u.Name}");
    }
    ...
    private void CheckEmailSentTo(string subject, string body, params string[] recipients)
    {
        foreach (var r in recipients) // should send to both users
        {
            ...
            _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is(MatchUser(address.DisplayName, address.Address, subject, body))),
                Times.Once,
                $"Should've sent message {subject} to {address}");
        }
    }
    ...
    {
        foreach (var u in users) // should send to both users
            _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is(MatchUser(u))), Times.Never, $"Shouldn't have sent any message to {u.Name}");
    }
    ...
    {
        return model => model.Subject == subject && model.HtmlBody.Contains(body) && model.Recipients.Any(r => r.Name == name && r.Email == email);
    }
