...
public class NotificationsControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    ...
    {
        using var scope = _factory.Services.NewTestScope();
        using var notification = await scope.CreateEmailNotificationAsync(totalSent: 10, totalErrors: 2);
    ...
    public async Task List_Should_Not_Limit_Output_For_Power_Admin(string role)
    {
        ...
        var token = await response.CheckOk().AsTokenAsync();
        token.CheckPaging((t, n) => t.CheckNotification(n), n4.Entity, n3.Entity, n2.Entity, n1.Entity);
    }
    ...
    public async Task List_Should_Use_Paging_Query_Params()
    {
        ...
        var token = await response.CheckOk().AsTokenAsync();
        token.CheckPaging(1, 3, (t, n) => t.CheckNotification(n), n3.Entity, n2.Entity);
    ...
    public async Task List_Should_Use_Sorting_Query_Params()
    {
        ...
        var token = await response.CheckOk().AsTokenAsync();
        token.CheckPaging((t, n) => t.CheckNotification(n), n3.Entity, n2.Entity, n1.Entity);
        ...
        token = await response.CheckOk().AsTokenAsync();
        token.CheckPaging((t, n) => t.CheckNotification(n), n1.Entity, n2.Entity, n3.Entity);
    ...
    {
        using var scope = _factory.Services.NewTestScope();
        using var notification = await scope.CreateEmailNotificationAsync(totalSent: 1, totalErrors: 0);

        var client = _factory.CreateClient().AuthenticatedAsSuperAdmin();
        var response = await client.GetAsync("/v3/notifications?includeStatistics=true");
        var token = await response.CheckOk().AsTokenAsync();
        token.CheckPaging((t, n) => t.CheckNotification(n, 1, 0), notification.Entity);
    }
}