...
public class SmsNotificationsControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>, IDisposable
{
    ...
    public async Task Should_Not_Send_Any_Sms_Notification_If_No_Registrants_Found()
    {
        ...
        _factory.SmsSenderMock.Verify(s => s.SendSmsAsync(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
    }
    ...
    public async Task Should_Not_Send_Sms_Notifications_To_Inaccessible_Events()
    {
        ...
        var response = await client.PostAsync($"/v3/notifications/sms?orgId={org.Entity.OrganizationId}",
            new
            {
        ...
        response = await client.PostAsync($"/v3/notifications/sms?orgId={org.Entity.OrganizationId}",
            new
            {
        ...
        CheckSmsSentTo("Test message 2", user.Entity, otherUser.Entity); // should send to both users
    ...
    public async Task Should_Send_Sms_Notifications_To_More_Than_100_Users()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                _factory.SmsSenderMock.Verify(s => s.SendSmsAsync(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(users.Length));
            }
            finally
            {
    ...
    public async Task Should_Send_Sms_Notifications_To_All_Users_For_Power_Admin(string role)
    {
        ...
        CheckSmsSentTo("Test message 1", u1.Entity, u2.Entity); // both users registered to event 1

        _factory.SmsSenderMock.Reset();

        response = await client.PostAsync("/v3/notifications/sms",
            new
            {
                message = "Test message 2",
                eventParticipants = new
        ...
        CheckSmsSentTo("Test message 2", u2.Entity); // only user 2 is registered to event 2
    ...
    public async Task Should_Send_Sms_Notifications_To_The_Given_Recipients_For_Admin(string role)
    {
        ...
        _factory.SmsSenderMock.Verify(s => s.SendSmsAsync("+11111111111", "Test message"), Times.Once);

        _factory.SmsSenderMock.Verify(s => s.SendSmsAsync("+22222222222", "Test message"), Times.Once);
    }
    ...
    public async Task Sms_Notifications_Should_Use_Status_And_Type_Filter(string role)
    {
        ...
        CheckSmsSentTo("Message to everyone", u1.Entity, u2.Entity, u3.Entity);
    }
    ...
    public async Task Should_Set_Error_For_Sms_Recipient_In_Case_Of_A_Delivery_Failure()
    {
        ...
        smsSenderMock.Reset();
        smsSenderMock.Setup(s => s.SendSmsAsync(It.IsAny<string>(), It.IsAny<string>())).ThrowsAsync(new Exception("test error"));
    ...
    private void CheckSmsSentTo(string message, params ApplicationUser[] users)
    {
        foreach (var u in users) // should send to both users
            _factory.SmsSenderMock.Verify(s => s.SendSmsAsync(u.PhoneNumber, message),
                Times.Once,
                $"Should've sent message {message} to {u.PhoneNumber}");
    }

    private void CheckSmsSentTo(string message, params string[] recipients)
    {
        foreach (var r in recipients) // should send to both users
            _factory.SmsSenderMock.Verify(s => s.SendSmsAsync(r, message), Times.Once, $"Should've sent message {message} to {r}");
    }

    private void CheckEmailNotSentTo(params ApplicationUser[] users)
    {
        foreach (var u in users) // should send to both users
            _factory.SmsSenderMock.Verify(s => s.SendSmsAsync(u.PhoneNumber, It.IsAny<string>()),
                Times.Never,
                $"Shouldn't have sent any message to {u.PhoneNumber}");
    }
