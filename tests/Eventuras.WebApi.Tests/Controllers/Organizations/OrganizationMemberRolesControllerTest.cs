...
public class OrganizationMemberRolesControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    ...
    public async Task Should_Return_Empty_List_If_Not_A_Member_Of_The_Org()
    {
        ...
        var response = await client.GetAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}/roles");
        response.CheckOk();
    ...
    public async Task Should_Return_List_Of_Roles()
    {
        ...
        using var user = await scope.CreateUserAsync();
        using var member = await scope.CreateOrganizationMemberAsync(user.Entity, org.Entity, roles: new[] { Roles.Admin, Roles.SuperAdmin });
        using var anotherMember = await scope.CreateOrganizationMemberAsync(user.Entity, anotherOrg.Entity, roles: new[] { Roles.SystemAdmin });

        var response = await client.GetAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}/roles");
        response.CheckOk();

        var json = await response.AsArrayAsync();
        json.CheckArray((token, role) => Assert.Equal(role, token.ToString()), Roles.Admin, Roles.SuperAdmin);

        response = await client.GetAsync($"/v3/organizations/{anotherOrg.Entity.OrganizationId}/members/{user.Entity.Id}/roles");
        response.CheckOk();

        json = await response.AsArrayAsync();
        json.CheckArray((token, role) => Assert.Equal(role, token.ToString()), Roles.SystemAdmin);
    }
    ...
    public async Task Add_Should_Validate_Role(string value)
    {
        ...
        using var org = await scope.CreateOrganizationAsync();
        using var user = await scope.CreateUserAsync();
        var response = await client.PostAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}/roles",
            new
            {
                role = value,
            });
    ...
    public async Task Should_Add_New_Role()
    {
        ...
        var updated = await scope.Db.OrganizationMembers.AsNoTracking()
            .Include(m => m.Roles)
            .SingleOrDefaultAsync(m => m.OrganizationId == org.Entity.OrganizationId && m.UserId == user.Entity.Id);
    ...
    public async Task Should_Return_Ok_If_Role_Already_Added()
    {
        ...
        using var org = await scope.CreateOrganizationAsync();
        using var user = await scope.CreateUserAsync();
        using var member = await scope.CreateOrganizationMemberAsync(user.Entity, org.Entity, role: Roles.Admin);
    ...
    public async Task Remove_Should_Validate_Role(string value)
    {
        ...
        using var org = await scope.CreateOrganizationAsync();
        using var user = await scope.CreateUserAsync();
        var response = await client.DeleteAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}/roles",
            new
            {
                role = value,
            });
    ...
    public async Task Should_Remove_Role()
    {
        ...
        using var user = await scope.CreateUserAsync();
        var member = await scope.CreateOrganizationMemberAsync(user.Entity, org.Entity, role: Roles.Admin);
        Assert.Single(member.Entity.Roles);
        Assert.Equal(Roles.Admin, member.Entity.Roles.First().Role);
