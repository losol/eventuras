...
public class OrganizationMembersControllerTest : IClassFixture<CustomWebApiApplicationFactory<Startup>>
{
    private readonly CustomWebApiApplicationFactory<Startup> _factory;

    public OrganizationMembersControllerTest(CustomWebApiApplicationFactory<Startup> factory)
    {
    ...
    public async Task Should_Return_Not_Found_When_Adding_Unknown_Member_To_Existing_Org()
    {
        ...
        var response = await client.PutAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/any", new { });
        response.CheckNotFound();
    }
    ...
    public async Task Should_Add_New_Member_To_Org()
    {
        ...
        using var user = await scope.CreateUserAsync();
        using var org = await scope.CreateOrganizationAsync();
        var response = await client.PutAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}", new { });
        response.CheckOk();

        var member = await scope.Db.OrganizationMembers.AsNoTracking()
            .FirstOrDefaultAsync(m => m.OrganizationId == org.Entity.OrganizationId && m.User.Id == user.Entity.Id);
    ...
    public async Task Should_Return_Ok_When_Adding_Member_To_Org_Second_Time()
    {
        ...
        var member = await scope.Db.OrganizationMembers.AsNoTracking()
            .SingleOrDefaultAsync(m => m.OrganizationId == org.Entity.OrganizationId && m.User.Id == user.Entity.Id);
    ...
    public async Task Should_Return_Not_Found_When_Removing_Unknown_Member_From_Existing_Org()
    {
        ...
        using var scope = _factory.Services.NewTestScope();
        using var org = await scope.CreateOrganizationAsync();
        var response = await client.DeleteAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/any");
        response.CheckNotFound();
    }
    ...
    public async Task Should_Remove_Member_From_Org()
    {
        ...
        var response = await client.DeleteAsync($"/v3/organizations/{org.Entity.OrganizationId}/members/{user.Entity.Id}");
        response.CheckOk();
    ...
    public async Task Should_Return_Ok_When_Removing_Member_From_Org_Second_Time()
    {
        ...
        Assert.False(await scope.Db.OrganizationMembers.AnyAsync(m => m.OrganizationId == org.Entity.OrganizationId && m.User.Id == user.Entity.Id));
    }
}