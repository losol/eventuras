...
public static class TestServiceScopeExtensions
{
        ...
        bool archived = false)
    {
        ...
        if (email == TestingConstants.Placeholder) email = $"{Guid.NewGuid()}@email.com";

        if (name == TestingConstants.Placeholder) name = email;

        if (password == TestingConstants.Placeholder) password = TestingConstants.DefaultPassword;

        if (phone == TestingConstants.Placeholder) phone = $"+{DateTimeOffset.Now.ToUnixTimeMilliseconds():####}";

        if (roles == null && !string.IsNullOrEmpty(role)) roles = new[] { role };
        ...
        if (organization != null) await scope.CreateOrganizationMemberAsync(user, organization, role: role, roles: roles);
    ...
    {
        if (name == TestingConstants.Placeholder) name = $"Test Collection {Guid.NewGuid()}";

        organizationId ??= (organization ?? (await scope.CreateOrganizationAsync()).Entity).OrganizationId;

        if (featuredImageUrl == TestingConstants.Placeholder) featuredImageUrl = featured ? $"http://some.featured.image.url/{Guid.NewGuid()}" : null;
    ...
    public static async Task<IDisposableEntity<EventInfo>> CreateEventAsync(
        ...
        bool archived = false)
    {
        if (title == TestingConstants.Placeholder) title = $"Test Event {Guid.NewGuid()}";

        if (description == TestingConstants.Placeholder) description = $"Test Event Description {Guid.NewGuid()}";

        if (slug == TestingConstants.Placeholder) slug = Guid.NewGuid().ToString();

        if (city == TestingConstants.Placeholder) city = "Oslo";

        if (collections == null && collection != null) collections = new[] { collection };
    ...
    {
        if (name == TestingConstants.Placeholder) name = $"Test Product {Guid.NewGuid()}";

        if (description == TestingConstants.Placeholder) description = $"Test Product Description {Guid.NewGuid()}";
    ...
    public static async Task<IDisposableEntity<ProductVariant>> CreateProductVariantAsync(
        ...
        bool archived = false)
    {
        if (name == TestingConstants.Placeholder) name = $"Test Product Variant {Guid.NewGuid()}";

        if (description == TestingConstants.Placeholder) description = $"Test Product Variant Description {Guid.NewGuid()}";
    ...
    public static async Task<IDisposableEntity<Certificate>> CreateCertificateAsync(this TestServiceScope scope, Registration registration)
    {
    ...
    public static async Task<IDisposableEntity<Certificate>> CreateCertificateAsync(this TestServiceScope scope)
    {
        ...
        Instant? time = null)
    {
        ...
        if (status == Order.OrderStatus.Invoiced || status == Order.OrderStatus.Refunded) order.Status = Order.OrderStatus.Verified;

        if (status == Order.OrderStatus.Refunded) order.Status = Order.OrderStatus.Invoiced;

        if (status != Order.OrderStatus.Draft) order.Status = status;
    ...
    {
        if (externalServiceName == TestingConstants.Placeholder) externalServiceName = "Test";

        if (externalEventId == TestingConstants.Placeholder) externalEventId = Guid.NewGuid().ToString();
    ...
    public static async Task<IDisposableEntity<Organization>> CreateOrganizationAsync(
        ...
        bool inactive = false)
    {
        if (name == TestingConstants.Placeholder) name = $"Test Org {Guid.NewGuid()}";

        if (hostnames == null && hostname != null) hostnames = new[] { hostname };
        ...
        string role = null)
    {
        ...
        if (roles == null && role != null) roles = new[] { role };
    ...
    {
        if (TestingConstants.Placeholder.Equals(subject)) subject = $"Test email subject {Guid.NewGuid()}";

        if (TestingConstants.Placeholder.Equals(body)) body = $"Test email body {Guid.NewGuid()}";

        var recipients = new List<NotificationRecipient>();

        if (emailAddresses != null) recipients.AddRange(emailAddresses.Select(NotificationRecipient.Email));

        if (recipientUsers != null) recipients.AddRange(recipientUsers.Select(NotificationRecipient.Email));

        if (registrations != null) recipients.AddRange(registrations.Select(NotificationRecipient.Email));
    ...
    public static async Task<IDisposableEntity<SmsNotification>> CreateSmsNotificationAsync(
        ...
        IEnumerable<Registration> registrations = null)
    {
        if (TestingConstants.Placeholder.Equals(message)) message = $"Test SMS message {Guid.NewGuid()}";

        var recipients = new List<NotificationRecipient>();

        if (phones != null) recipients.AddRange(phones.Select(NotificationRecipient.Sms));

        if (recipientUsers != null) recipients.AddRange(recipientUsers.Select(NotificationRecipient.Sms));

        if (registrations != null) recipients.AddRange(registrations.Select(NotificationRecipient.Sms));
