...
public class ExternalEventsControllerTest : IClassFixture<CustomWebApplicationFactory<Startup>>
{
    ...
    public async Task Should_Return_External_Event_List_For_Event()
    {
        ...
        JArray.Parse(content)
            .CheckArray((token, externalEvent) => token.CheckExternalEvent(externalEvent), externalEvent.Entity, externalEvent2.Entity);
    }
    ...
    public async Task Delete_Should_Return_Not_Found_For_Wrong_Local_Id()
    {
        ...
        var response = await client.DeleteAsync(
            $"/api/v1/events/external/{eventInfo.Entity.EventInfoId:####}?localId={anotherExternalEvent.Entity.LocalId}");
        response.CheckNotFound();
    }
    ...
    public async Task Should_Delete_External_Event()
    {
        ...
        var response = await client.DeleteAsync(
            $"/api/v1/events/external/{eventInfo.Entity.EventInfoId:####}?localId={externalEvent.Entity.LocalId}");
        response.CheckOk();

        scope.Db.Entry(eventInfo.Entity.ExternalEvents.First()).State = EntityState.Detached;
        Assert.Null(await scope.Db.ExternalEvents.AsNoTracking().FirstOrDefaultAsync(c => c.LocalId == externalEvent.Entity.LocalId));
    }
}