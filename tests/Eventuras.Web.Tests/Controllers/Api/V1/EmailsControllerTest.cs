...
public class EmailsControllerTest : IClassFixture<CustomWebApplicationFactory<Startup>>
{
    ...
    public async Task Should_Not_Allow_To_Send_To_More_Than_500_Recipients()
    {
        ...
        var emails = Enumerable.Range(0, 501).Select(i => $"email{i}@test.com").ToArray();

        var response = await client.PostAsync("/api/v1/emails",
    ...
    public async Task Should_Allow_To_Send_To_Exactly_500_Recipients()
    {
        ...
        var emails = Enumerable.Range(0, 500).Select(i => $"email+{i}@test.com").ToArray();
        ...
        _factory.EmailSenderMock.Verify(s => s.SendEmailAsync(It.Is<EmailModel>(model
    ...
    public static object[][] InvalidRequestData =
    {
        // empty request
        new object[] { "" },
