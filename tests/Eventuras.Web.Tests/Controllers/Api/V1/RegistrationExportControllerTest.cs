...
public class RegistrationExportControllerTest : IClassFixture<CustomWebApplicationFactory<Startup>>
{
    ...
    public async Task Should_Export_All_Registrations(bool exportHeader)
    {
        ...
        using var r1 = await scope.CreateRegistrationAsync(e1.Entity, u1.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(3)));
        using var r2 = await scope.CreateRegistrationAsync(e1.Entity, u2.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(2)));
        using var r3 = await scope.CreateRegistrationAsync(e2.Entity, u2.Entity, time: SystemClock.Instance.Now().Minus(Duration.FromDays(1)));
    ...
    private static async Task CheckSuccessfulResponseAsync(HttpResponseMessage response, bool exportHeader, params Registration[] registrations)
    {
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    private static void CheckExcelDocument(Stream stream, bool checkHeader, params Registration[] registrations)
    {
        ...
        for (var i = 0; i < registrations.Length; ++i)
        {
            ...
            if (registration.Orders?.Any() == true)
            {
                ...
                {
                    Assert.Contains(product.Quantity > 1 ? $"{product.Quantity} × {product.Product.Name}" : product.Product.Name, productText);
                    if (product.Variant != null) Assert.Contains($"({product.Variant.Name})", productText);
                }
            }
        }
    }
