...
public class OrganizationSettingsRegistryTests
{
    ...
    public void RegisterSettings_Should_Define_Property_Types()
    {
        ...
        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.String)}" & e.Type == OrganizationSettingType.String);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Email)}" & e.Type == OrganizationSettingType.Email);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Url)}" & e.Type == OrganizationSettingType.Url);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Integer)}" & e.Type == OrganizationSettingType.Number);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Long)}" & e.Type == OrganizationSettingType.Number);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Short)}" & e.Type == OrganizationSettingType.Number);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Byte)}" & e.Type == OrganizationSettingType.Number);

        Assert.Contains(entries,
            e => e.Section == typeName
              && e.Name == $"{typeName}.{nameof(SettingsPocoWithAllPropertyTypes.Boolean)}" & e.Type == OrganizationSettingType.Boolean);
    }
}
