...
public class OrganizationSettingsAccessorServiceTests
{
    ...
    {
        _currentOrganizationAccessorServiceMock
            .Setup(s => s.RequireCurrentOrganizationAsync(It.IsAny<OrganizationRetrievalOptions>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(_org);

        _organizationSettingsCacheMock.Setup(s => s.GetAllSettingsForOrganizationAsync(_org.OrganizationId)).ReturnsAsync(_settings.ToArray);

        _service = new OrganizationSettingsAccessorService(_currentOrganizationAccessorServiceMock.Object, _organizationSettingsCacheMock.Object);
    }
    ...
    public async Task Should_Not_Convert_Invalid_Poco_Value_Types()
    {
        ...
        await Assert.ThrowsAsync<FormatException>(() => _service.ReadOrganizationSettingsAsync<SettingsPocoWithIntegerMappedField>());
    }

    [Fact]
    public async Task Should_Return_Empty_Poco_With_No_Mapped_Fields()
    {
        var poco = await _service.ReadOrganizationSettingsAsync<SettingsPocoWithNoFieldsMapped>();
    ...
    {
        var poco = await _service.ReadOrganizationSettingsAsync<SettingsPocoWithSingleOptionalMappedField>();
    ...
    {
        await Assert.ThrowsAsync<ValidationException>(() => _service.ReadOrganizationSettingsAsync<SettingsPocoWithSingleRequiredMappedField>());
    }
    ...
    public async Task Should_Return_Poco_From_Settings()
    {
        ...
        var poco = await _service.ReadOrganizationSettingsAsync<SettingsPocoWithSingleRequiredMappedField>();
    ...
    {
        var poco = await _service.ReadOrganizationSettingsAsync<ConfigurableSettingsPocoWithSingleRequiredField>();
    ...
    public async Task Should_Validate_Enabled_Setting()
    {
        _settings.Add(new OrganizationSetting
        {
            Name = $"{nameof(ConfigurableSettingsPocoWithSingleRequiredField)}.{nameof(ConfigurableSettingsPocoWithSingleRequiredField.Enabled)}",
            Value = "true",
        });
