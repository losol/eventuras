...
public class OrdersModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetOrdersAsync(int? id)
    {
        ...
        var orders = (await _registrations.GetAsync(id.Value)).Orders.SelectMany(o => o.OrderLines).Select(ol => ol.OrderLineId);
        return new JsonResult(orders);
    }

    public string GetOrderButtonText(Registration registration)
    {
        if (!registration.Orders.Any(o => o.Status != Order.OrderStatus.Invoiced)) return "New Order";
