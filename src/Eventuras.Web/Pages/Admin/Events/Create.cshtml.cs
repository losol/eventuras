...
public class CreateModel : PageModel
{
    ...
    public CreateModel(IEventManagementService eventsService, ICurrentOrganizationAccessorService currentOrganizationAccessorService)
    {
        _eventsService = eventsService;
        _currentOrganizationAccessorService = currentOrganizationAccessorService;
    }

    public async Task<IActionResult> OnGet() => await PageAsync();
    ...
    {
        if (!ModelState.IsValid) return await PageAsync();

        if (!User.IsInRole(Roles.SuperAdmin))
            // Force current org id to be set by all admins except
            // super admin who can choose not to bind event to the org.
            EventInfo.OrganizationId = (await _currentOrganizationAccessorService.RequireCurrentOrganizationAsync()).OrganizationId;
