...
public class ConfirmModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(int? id)
    {
        ...
        if (HttpContext.Request.Query["auth"] == registration.VerificationCode)
        {
            ...
            await _registrationEmailSender.SendRegistrationAsync("kurs@nordland-legeforening.no",
                $"KOPI: Påmelding {registration.EventInfo.Title}",
                adminmessage,
                registration.RegistrationId);

            // Send welcome letter to participant
            await _standardEmailSender.SendStandardEmailAsync($@"{registration.ParticipantName} <{registration.User.Email}>",
