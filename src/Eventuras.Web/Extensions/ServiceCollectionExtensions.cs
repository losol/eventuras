...
public static class ServiceCollectionExtensions
{
    ...
    public static void ConfigureMvc(this IServiceCollection services)
    {
        services
            .AddControllersWithViews(options
                => options.Filters.Add(
                    new HttpResponseExceptionFilter())) // See https://docs.microsoft.com/en-us/aspnet/core/web-api/handle-errors?view=aspnetcore-5.0#use-exceptions-to-modify-the-response
    ...
    public static void AddInvoicingServices(this IServiceCollection services, AppSettings appsettings, IConfiguration config)
    {
        if (appsettings.UsePowerOffice) services.AddPowerOffice(config.GetSection("PowerOffice"));

        if (appsettings.UseStripeInvoice) services.AddStripe(config.GetSection("Stripe"));
    }
    ...
    public static void AddApplicationHealthChecks(this IServiceCollection services, IConfigurationSection configuration)
    {
        ...
        services.AddHealthChecksUI(
                settings => { settings.AddHealthCheckEndpoint(Constants.HealthCheckName, $"{baseUri}{Constants.HealthCheckUri}"); })
            .AddInMemoryStorage();
    }
}