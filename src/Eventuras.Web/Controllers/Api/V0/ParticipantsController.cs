...
public class ParticipantsController : Controller
{
    ...
    public async Task<IActionResult> OrderEmails(
        [FromRoute] int eventId,
        [FromServices] IApplicationEmailSender emailSender,
        ...
        [FromBody] EmailVm vm)
    {
        ...
        var emailTasks = registrations.Select(r =>
        {
            ...
            if (r.HasOrder)
            {
                ...
                r.Orders.ForEach(o => o.OrderLines?.ForEach(line => builder.AppendLine($"<br>{line.ProductName}")));

                message += builder.ToString();
            }

            return emailSender.SendStandardEmailAsync($"{r.ParticipantName} <{r.User.Email}>", vm.Subject, message);
        });
