...
public class OrdersController : Controller
{
    ...
    public OrdersController(IOrderService orderService, IInvoicingService invoicingService, IOrderRetrievalService orderRetrievalService)
    {
    ...
    public async Task<ActionResult> UpdateOrderComment([FromRoute] int id, [FromBody] UpdateOrderDetailsVM vm)
    {
        ...
        try { await _orderService.UpdateOrderComment(id, vm.Comments); }
    ...
    public async Task<IActionResult> GetProductIdsForRegistration(
        [FromRoute] int registrationId,
        [FromServices] IRegistrationService registrationService)
    {
        var orders = (await registrationService.GetWithOrdersAsync(registrationId)).Orders.SelectMany(o => o.OrderLines).Select(ol => ol.ProductId);
        return Ok(orders);
    }
        ...
        try
        {
            switch (status)
            {
                ...
                case OrderStatus.Invoiced:
                    var order = await _orderRetrievalService.GetOrderByIdAsync(id, OrderRetrievalOptions.ForInvoicing());
    ...
    public async Task<ActionResult> SetPaymentMethod([FromRoute] int id, [FromRoute] PaymentProvider paymentmethod)
    {
        ...
        try { await _orderService.UpdatePaymentMethod(id, paymentmethod); }
    ...
    public async Task<ActionResult> Reopen([FromRoute] int id)
    {
        ...
        catch (InvalidOperationException) { return StatusCode(StatusCodes.Status409Conflict); }
    }
    ...
    public async Task<IActionResult> DeleteOrderLine(int lineid)
    {
        ...
        if (!line.Order.CanEdit) return BadRequest();

        return await _orderService.DeleteOrderLineAsync(lineid) ? Ok() : StatusCode(StatusCodes.Status500InternalServerError);
    }
    ...
    public async Task<IActionResult> AddOrderLine([FromBody] OrderLineVM vm)
    {
        ...
        try { await _orderService.AddOrderLineAsync(vm.OrderId, vm.ProductId, vm.VariantId); }
        catch (ArgumentException) { return BadRequest(); }
    ...
    public async Task<IActionResult> UpdateOrderLine([FromRoute] int lineId, [FromBody] OrderLineUpdateVM vm)
    {
        ...
        try { await _orderService.UpdateOrderLine(lineId, vm.VariantId, vm.Quantity, vm.Price); }
        catch (ArgumentException) { return BadRequest(); }
    ...
    public async Task<IActionResult> Update([FromBody] UpdateOrderVM vm, [FromServices] IRegistrationService registrationService)
    {
        if (!ModelState.IsValid) return BadRequest();

        try
        {
            await registrationService.CreateOrUpdateOrder(vm.RegistrationId,
                vm.Products.Select(o => new OrderVM
                    {
                        ProductId = o.Id,
