...
public class ProductsController : Controller
{
    ...
    [HttpPost("add-user")]
    public async Task<IActionResult> AddUserToProduct([FromBody] AddUserToProductVM vm, [FromServices] IRegistrationService registrationService)
    {
        if (!ModelState.IsValid) return BadRequest();

        try { await registrationService.CreateOrUpdateOrder(vm.RegistrationId, vm.ProductId, vm.ProductVariantId); }
