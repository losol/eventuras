...
namespace Eventuras.Web.Controllers.Api.V1
{
    ...
    public class EmailsController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendEmailsAsync([FromBody] BulkEmailRequest request)
        {
            ...
            var from = request.From != null
                ? NewAddress(request.From.Name, request.From.Email)
                : null;

            var cc = request.Cc?
                .Select(dto => NewAddress(dto.Name, dto.Email))
                .ToArray();
        ...
        private static Address NewAddress(string name, string email)
        {
    ...
    public class ValidContentAttribute : ValidationAttribute
    {
        ...
        {
            if (!(value is BulkEmailContentDto content) ||
                string.IsNullOrEmpty(content.Text) && string.IsNullOrEmpty(content.Html))
            {
    ...
    public class BulkEmailResponse
    {
        ...
        public List<BulkEmailResponseError> Errors { get; } = new List<BulkEmailResponseError>();
    }
}
