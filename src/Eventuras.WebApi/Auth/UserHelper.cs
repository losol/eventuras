...
public static class UserHelpers
{
    ...
    {
        var standardClaimEmail = principal.FindFirst(ClaimTypes.Email);
        if (standardClaimEmail != null && !string.IsNullOrEmpty(standardClaimEmail.Value)) return standardClaimEmail.Value;

        var userEmailClaim = principal.FindFirst(Constants.Auth.EmailClaimType);
        if (userEmailClaim != null && !string.IsNullOrEmpty(userEmailClaim.Value)) return userEmailClaim.Value;
