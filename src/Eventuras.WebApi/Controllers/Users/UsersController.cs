...
public class UsersController : Controller
{
    ...
    public UsersController(IUserRetrievalService userRetrievalService, IUserManagementService userManagementService)
    {
    ...
    public async Task<IActionResult> Me(CancellationToken cancellationToken)
    {
        ...
        catch (NotFoundException) { user = await _userManagementService.CreateNewUserAsync(nameClaim, emailClaim, phoneClaim); }
    ...
    public async Task<IActionResult> Get(string id, CancellationToken cancellationToken)
    {
        ...
        if (!principal.IsAdmin() && id != principal.GetUserId()) return Forbid();
    ...
    public async Task<PageResponseDto<UserDto>> List([FromQuery] UsersQueryDto request, CancellationToken cancellationToken)
    {
        var paging = await _userRetrievalService.ListUsers(new UserListRequest
            {
        ...
        return PageResponseDto<UserDto>.FromPaging(request, paging, u => new UserDto(u));
    }
    ...
    public async Task<IActionResult> CreateNewUser([FromBody] NewUserDto dto, CancellationToken cancellationToken)
    {
        ...
        var user = await _userManagementService.CreateNewUserAsync(dto.Name, dto.Email, dto.PhoneNumber, cancellationToken);
