...
public class CertificatesController : ControllerBase
{
    ...
    public CertificatesController(ICertificateRetrievalService certificateRetrievalService, ICertificateRenderer certificateRenderer)
    {
    ...
    {
        var cert = await _certificateRetrievalService.GetCertificateByIdAsync(id, CertificateRetrievalOptions.ForRendering);

        format ??= GetCertificateFormatFromMediaType(Request.Headers[HeaderNames.Accept]) ?? CertificateFormat.Json;

        switch (format)
        {
            case CertificateFormat.Json: return Ok(new CertificateDto(cert));

            case CertificateFormat.Html:
                var html = await _certificateRenderer.RenderToHtmlAsStringAsync(new CertificateViewModel(cert));
            ...
            default: throw new InvalidOperationException($"Unsupported cert format: {format}");
        }
    }
    ...
    private static CertificateFormat? GetCertificateFormatFromMediaType(params string[] mediaTypes)
    {
        ...
        {
            if (MediaTypeNames.Text.Html.ToLower().Equals(mediaType)) return CertificateFormat.Html;

            if (MediaTypeNames.Application.Pdf.ToLower().Equals(mediaType)) return CertificateFormat.Pdf;

            if (MediaTypeNames.Application.Json.ToLower().Equals(mediaType)) return CertificateFormat.Json;
        }
