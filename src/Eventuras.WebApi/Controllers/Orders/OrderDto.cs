...
public class OrderDto
{
    ...
    public OrderDto(Order order)
    {
        ...
        if (order.Registration != null) Registration = new OrderRegistrationDto(order.Registration);

        if (order.User != null) User = new UserDto(order.User);

        Items = order.OrderLines?.Select(l => new OrderLineDto(l)).ToArray();
    }
}