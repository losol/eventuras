...
public class OrganizationSettingValueAttribute : ValidationAttribute
{
    ...
    {
        var service = (IOrganizationSettingsRegistry)validationContext.GetService(typeof(IOrganizationSettingsRegistry));
                ...
                switch (setting.Type)
                {
                    case OrganizationSettingType.Number:
                        if (!double.TryParse(dto.Value, out _))
                            return new ValidationResult($"Invalid value for {dto.Name}: {dto.Value} (number expected)");

                        break;

                    case OrganizationSettingType.Url:
                        if (!Uri.TryCreate(dto.Value, UriKind.Absolute, out _))
                            return new ValidationResult($"Invalid value for {dto.Name}: {dto.Value} (URL expected)");

                        break;

                    case OrganizationSettingType.Email:
                        if (!MailAddress.TryCreate(dto.Value, out _))
                            return new ValidationResult($"Invalid value for {dto.Name}: {dto.Value} (email address expected)");

                        break;

                    case OrganizationSettingType.Boolean:
                        if (!bool.TryParse(dto.Value, out _))
                            return new ValidationResult($"Invalid value for {dto.Name}: {dto.Value} (boolean true/false expected)");
