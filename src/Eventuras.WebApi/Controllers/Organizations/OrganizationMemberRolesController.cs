...
public class OrganizationMemberRolesController : ControllerBase
{
    ...
    public async Task<string[]> List(int organizationId, string userId, CancellationToken token)
    {
        ...
        return user.OrganizationMembership.FirstOrDefault(m => m.OrganizationId == organizationId)
    ...
    {
        return await ManipulateRolesAsync(organizationId,
            userId,
            dto.Role,
            (roles, r) =>
            {
    ...
    private async Task<string[]> ManipulateRolesAsync(int organizationId, string userId, string role, Func<ICollection<string>, string, bool> f)
    {
        await _organizationRetrievalService.GetOrganizationByIdAsync(organizationId); // just to check its existence
        ...
        var m = user.OrganizationMembership.FirstOrDefault(m => m.OrganizationId == organizationId);

        if (m == null) throw new NotFoundException($"User {userId} is not a member of organization {organizationId}");

        var roles = m.Roles.Select(r => r.Role).ToHashSet();

        if (f(roles, role)) await _rolesManagementService.UpdateOrganizationMemberRolesAsync(m.Id, roles.ToArray());
