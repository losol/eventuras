...
public class RegistrationCertificateController : ControllerBase
{
    ...
    public async Task<IActionResult> Send(int id, CancellationToken cancellationToken)
    {
        ...
        if (reg.Certificate == null) return NotFound($"Registration {id} doesn't have certificate issued");

        await _certificateDeliveryService.SendCertificateAsync(reg.Certificate, cancellationToken);
