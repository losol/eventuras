...
public class NotificationsQueueingController : ControllerBase
{
    ...
    [HttpPost("email")]
    public async Task<ActionResult<NotificationDto>> SendEmail(EmailNotificationDto dto, CancellationToken cancellationToken)
    {
        EmailNotification emailNotification;
        var eventFilter = await GetEventParticipantFilterAsync(dto, cancellationToken);
        if (eventFilter != null)
            emailNotification = await _notificationManagementService.CreateEmailNotificationForEventAsync(dto.Subject,
                dto.BodyMarkdown,
            ...
            emailNotification = await _notificationManagementService.CreateEmailNotificationAsync(dto.Subject, dto.BodyMarkdown, dto.Recipients);

        // send notification right away, not queueing it or something.
        // TODO: use queue for messages

        await _notificationDeliveryService.SendNotificationAsync(emailNotification, cancellationToken);

        return Ok(new NotificationDto(emailNotification));
    }

    [HttpPost("sms")]
    public async Task<ActionResult<NotificationDto>> SendSms(SmsNotificationDto dto, CancellationToken cancellationToken)
    {
        SmsNotification smsNotification;
        var eventFilter = await GetEventParticipantFilterAsync(dto, cancellationToken);
        if (eventFilter != null)
            smsNotification = await _notificationManagementService.CreateSmsNotificationForEventAsync(dto.Message,
                eventFilter.EventId.Value,
                eventFilter.ProductId,
                eventFilter.RegistrationStatuses,
                eventFilter.RegistrationTypes);
        else
            smsNotification = await _notificationManagementService.CreateSmsNotificationAsync(dto.Message, dto.Recipients);

        // send notification right away, not queueing it or something.
        // TODO: use queue for messages

        await _notificationDeliveryService.SendNotificationAsync(smsNotification, cancellationToken);

        return Ok(new NotificationDto(smsNotification));
    }

    private async Task<EventParticipantsFilterDto> GetEventParticipantFilterAsync(INotificationDto dto, CancellationToken cancellationToken = default)
    {
        ...
        if (eventFilterSet)
        {
            ...
            {
                var product = await _productRetrievalService.GetProductByIdAsync(eventFilter.ProductId.Value, cancellationToken: cancellationToken);

                if (eventFilter.EventId.HasValue)
                {
                    if (product.EventInfoId != eventFilter.EventId.Value)
                        throw new InputException($"Product {product.ProductId} doesn't belong to event {eventFilter.EventId}");
                }
                else { eventFilter.EventId = product.EventInfoId; }
            }
