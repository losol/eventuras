...
public class EventProductsController : ControllerBase
{
        ...
        IEventInfoAccessControlService eventInfoAccessControlService)
    {
        ...
        _eventProductsManagementService = eventProductsManagementService ?? throw new ArgumentNullException(nameof(eventProductsManagementService));
    ...
    [HttpGet]
    [AllowAnonymous]
    public async Task<ProductDto[]> List(int eventId, [FromQuery] EventProductsQueryDto query, CancellationToken token)
    {
        ...
        var products = await _productRetrievalService.ListProductsAsync(new ProductListRequest(eventId)
            {
    ...
    public async Task<IActionResult> Add(int eventId, [FromBody] NewProductDto dto)
    {
        ...
        await _eventProductsManagementService.AddProductAsync(product);
    ...
    private async Task<Product> GetProductAsync(int eventId, int productId)
    {
        ...
        if (product.EventInfoId != eventId) throw new NotFoundException($"Product {productId} is not found for event {eventId}");
