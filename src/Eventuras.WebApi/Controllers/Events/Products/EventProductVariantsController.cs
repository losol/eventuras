...
public class EventProductVariantsController : ControllerBase
{
    ...
    public async Task<ProductVariantDto[]> List(int eventId, int productId, CancellationToken token)
    {
        ...
        return product.ProductVariants?.Where(v => !v.Archived).OrderBy(v => v.Name).Select(v => new ProductVariantDto(v)).ToArray()
    ...
    public async Task Archive(int eventId, int productId, int id)
    {
        ...
        var variant = product.ProductVariants.FirstOrDefault(v => v.ProductVariantId == id)
    ...
    private async Task<Product> GetProductAsync(int eventId, int productId, bool forUpdate = false, CancellationToken token = default)
    {
        ...
        return eventInfo.Products.FirstOrDefault(p => p.ProductId == productId)
