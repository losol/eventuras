...
internal class ConvertoClient : IConvertoClient
{
    ...
    public ConvertoClient(IHttpClientFactory httpClientFactory, IOptions<ConvertoConfig> options, ILogger<ConvertoClient> logger)
    {
        _options = options;
        _logger = logger;
    ...
    public async Task<Stream> Html2PdfAsync(string html, float scale, string format)
    {
        ...
        try
        {
            ...
            var response = await client.PostAsync(endpointUrl,
            ...
            if (!response.IsSuccessStatusCode) throw new ConvertoClientException($"{endpointUrl} returned {response.StatusCode} status code");

            return await response.Content.ReadAsStreamAsync();
        }
        catch (IOException e) { throw new ConvertoClientException($"Failed to authenticate using endpoint {endpointUrl}", e); }
    }
}