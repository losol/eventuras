...
public class PowerOfficeService : IInvoicingProvider
{
    ...
    public bool AcceptPaymentProvider(PaymentProvider provider)
        => provider is PaymentProvider.PowerOfficeEmailInvoice or PaymentProvider.PowerOfficeEHFInvoice;
    ...
    private async Task<Go> GetApiAsync()
    {
        ...
        var appKey = await _organizationSettingsAccessorService.GetOrganizationSettingByNameAsync(PowerOfficeConstants.ApplicationKey)
                  ?? _options.Value.ApplicationKey;

        var clientKey = await _organizationSettingsAccessorService.GetOrganizationSettingByNameAsync(PowerOfficeConstants.ClientKey)
    ...
    public async Task<InvoiceResult> CreateInvoiceAsync(InvoiceInfo info)
    {
        ...
        if (api.Client == null) throw new InvoicingException("Did not find PowerOffice Client");

        var result = new InvoiceResult();
        var customer = await CreateCustomerIfNotExistsAsync(info, result);
        _logger.LogInformation("* Bruker kunde med epost: {CustomerEmailAddress}, id {CustomerId}", customer.EmailAddress, customer.Id);
        ...
        if (!string.IsNullOrWhiteSpace(info.ProjectCode)) invoice.ProjectCode = info.ProjectCode;
    ...
    private async Task<Customer> CreateCustomerIfNotExistsAsync(InvoiceInfo info, InvoiceResult result)
    {
        ...
        var existingCustomer = !string.IsNullOrWhiteSpace(vatNumber)
            ? api.Customer.Get().FirstOrDefault(c => c.VatNumber == info.CustomerVatNumber)
            : null;
        ...
        if (!string.IsNullOrWhiteSpace(customerEmail)) existingCustomer ??= api.Customer.Get().FirstOrDefault(c => c.EmailAddress == customerEmail);

        // If we found a customer, return him!
        if (existingCustomer != null)
        {
            result.LogEntries.Add($"Kunden {existingCustomer.Name} med epost {existingCustomer.EmailAddress} fantes allerede.");
    ...
    {
        foreach (var line in info.Lines.Where(l => l.Type == InvoiceLineType.Product)) await CreateProductIfNotExistsAsync(line);
    }
}