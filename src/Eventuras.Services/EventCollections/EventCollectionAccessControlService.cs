...
public class EventCollectionAccessControlService : IEventCollectionAccessControlService
{
    ...
    public Task CheckEventCollectionReadAccessAsync(EventCollection collection, CancellationToken cancellationToken = default)
        => Task.CompletedTask; // anyone can read any collection for now 

    public async Task CheckEventCollectionUpdateAccessAsync(EventCollection collection, CancellationToken cancellationToken = default)
    {
        var principal = _httpContextAccessor.HttpContext.User;
        if (!principal.IsAdmin()) throw new NotAccessibleException("Event collection may be updated by admin only.");

        if (principal.IsPowerAdmin()) return;
        ...
        if (collection.OrganizationId != organization.OrganizationId)
            throw new NotAccessibleException($"Event collection {collection.CollectionId} is not available for update.");
    }
    ...
    {
        var principal = _httpContextAccessor.HttpContext.User;
        if (principal.IsPowerAdmin() || principal.IsAnonymous()) return query;

        var organization = await _currentOrganizationAccessorService.GetCurrentOrganizationAsync(cancellationToken: cancellationToken);
