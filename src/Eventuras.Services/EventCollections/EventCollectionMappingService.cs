...
internal class EventCollectionMappingService : IEventCollectionMappingService
{
    ...
    public async Task AddEventToCollectionAsync(int eventId, int collectionId, CancellationToken cancellationToken)
    {
        ...
        try { await _context.CreateAsync(mapping, cancellationToken: cancellationToken); }
        ...
        catch (DbUpdateException e) when (e.IsForeignKeyViolation())
        {
            ...
            _logger.LogError(e, "Failed to map event {eventId} to collection {collectionId}", eventId, collectionId);
    ...
    {
        var @event = await _context.EventInfos.FirstOrDefaultAsync(e => e.EventInfoId == eventId, cancellationToken);
        if (@event == null) throw new NotFoundException($"Event {eventId} not found");

        var collection = await _context.EventCollections.FirstOrDefaultAsync(c => c.CollectionId == collectionId, cancellationToken);
        if (collection == null) throw new NotFoundException($"Event collection {collectionId} not found");
        ...
        var mapping = await _context.EventCollectionMappings.AsNoTracking()
            .FirstOrDefaultAsync(m => m.EventId == eventId && m.CollectionId == collectionId, cancellationToken);
        return mapping;
    }
    ...
    private async Task<bool> IsAccessibleForEditAsync(int organizationId, CancellationToken cancellationToken)
    {
        ...
        if (principal.IsPowerAdmin()) return true;
