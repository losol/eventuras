...
public class EventCollectionRetrievalService : IEventCollectionRetrievalService
{
    ...
    public EventCollectionRetrievalService(ApplicationDbContext context, IEventCollectionAccessControlService eventCollectionAccessControlService)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var query = _context.EventCollections.UseOptions(options ?? new EventCollectionRetrievalOptions())
            .UseFilter(request.Filter ?? new EventCollectionFilter())
            .UseOrder(request.Order, request.Descending);

        query = await _eventCollectionAccessControlService.AddAccessFilterAsync(query, cancellationToken);

        return await query.ToArrayAsync(cancellationToken);
    }

    public async Task<EventCollection> GetCollectionByIdAsync(int id, EventCollectionRetrievalOptions options, CancellationToken cancellationToken)
    {
        options ??= new EventCollectionRetrievalOptions();

        var collection = await _context.EventCollections.UseOptions(options).FirstOrDefaultAsync(c => c.CollectionId == id, cancellationToken);

        if (options.ForUpdate) await _eventCollectionAccessControlService.CheckEventCollectionUpdateAccessAsync(collection, cancellationToken);
