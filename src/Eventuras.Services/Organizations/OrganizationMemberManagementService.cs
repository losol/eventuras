...
internal class OrganizationMemberManagementService : IOrganizationMemberManagementService
{
        ...
        OrganizationMemberRetrievalOptions options = null)
    {
        ...
        organization ??= await _currentOrganizationAccessorService.GetCurrentOrganizationAsync();
        ...
        OrganizationMemberRetrievalOptions options = null)
    {
        ...
        organization ??= await _currentOrganizationAccessorService.RequireCurrentOrganizationAsync();

        await CheckOrganizationAdminAccessAsync(organization);

        var member = await FindExistingMemberAsync(organization, user, options ?? new OrganizationMemberRetrievalOptions());
        ...
        if (member == null)
            ...
            catch (DbUpdateException e) when (e.IsUniqueKeyViolation())
            {
                ...
                if (member != null) _context.OrganizationMembers.Remove(member);
                return await FindExistingMemberAsync(organization, user, options);
            }
    ...
    public async Task RemoveFromOrganizationAsync(ApplicationUser user, Organization organization)
    {
        if (user == null) throw new ArgumentNullException(nameof(user));
        ...
        OrganizationMemberRetrievalOptions options = null)
    {
        return await _context.OrganizationMembers.AsNoTracking()
            ...
            .FirstOrDefaultAsync(m => m.OrganizationId == organization.OrganizationId && m.UserId == user.Id);
    }
    ...
    private async Task CheckOrganizationAdminAccessAsync(Organization organization)
    {
        ...
        if (string.IsNullOrWhiteSpace(userId)) throw new NotAccessibleException("Not authenticated.");

        if (!principal.IsAdmin()) throw new NotAccessibleException($"Should have at least {Roles.Admin} role to manage org membership.");

        if (!principal.IsPowerAdmin()
         && !await _context.OrganizationMembers.AnyAsync(m => m.UserId == userId && m.OrganizationId == organization.OrganizationId))
