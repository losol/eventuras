...
internal class OrganizationManagementService : IOrganizationManagementService
{
    ...
    public async Task CreateNewOrganizationAsync(Organization organization)
    {
        ...
        await _organizationAccessControlService.CheckOrganizationUpdateAccessAsync(organization.OrganizationId);

        try { await _context.CreateAsync(organization); }
    ...
    public async Task UpdateOrganizationHostnames(int id, string[] hostnames)
    {
        ...
        var organization = await _context.Organizations.Include(o => o.Hostnames).SingleOrDefaultAsync(o => o.OrganizationId == id)
                        ?? throw new NotFoundException($"Organization {id} not found");

        if (!organization.Active) throw new NotFoundException($"Organization {id} is deleted");

        var normalizedHostnames = hostnames.Select(NormalizeHostname).Where(h => !string.IsNullOrWhiteSpace(h)).Distinct().ToArray();

        foreach (var hostname in normalizedHostnames)
            if (await _context.OrganizationHostnames.AnyAsync(h => h.OrganizationId != id && h.Active && h.Hostname == hostname))
                throw new DuplicateException($"Duplicate org hostname: {hostname}");

        var oldHostnames = organization.Hostnames.ToDictionary(h => h.Hostname);
    ...
    public async Task DeleteOrganizationAsync(int id)
    {
        ...
        if (!organization.Active) return; // already deleted
    ...
    private static string NormalizeHostname(string h)
    {
        ...
        if (!h.StartsWith("http") || !h.Contains("://")) return h;
