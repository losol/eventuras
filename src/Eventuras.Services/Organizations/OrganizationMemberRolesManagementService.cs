...
internal class OrganizationMemberRolesManagementService : IOrganizationMemberRolesManagementService
{
    ...
    public async Task UpdateOrganizationMemberRolesAsync(int memberId, string[] roles)
    {
        ...
        var member = await _context.OrganizationMembers.Include(m => m.Roles).SingleAsync(m => m.Id == memberId);

        var oldRoles = member.Roles.ToDictionary(r => r.Role);
        ...
        foreach (var role in oldRoles.Values) member.Roles.Remove(role);
