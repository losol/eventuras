...
{
    private readonly IDictionary<string, OrganizationSettingEntry> _entries = new Dictionary<string, OrganizationSettingEntry>();

    public void RegisterSetting(string name, string section, string description, OrganizationSettingType type)
    {
        ...
        if (_entries.ContainsKey(key)) throw new DuplicateException($"Duplicate settings key {name} in section {section}");
    ...
    public void RegisterSettings<T>(string section)
    {
        ...
        foreach (var property in type.GetProperties())
        {
            ...
            if (orgSettingKey != null) settingKey = orgSettingKey.Name;

            var settingDescription = property.Name;
            var propertyDisplayName = property.GetCustomAttribute<DisplayNameAttribute>();
            if (propertyDisplayName != null) settingDescription = propertyDisplayName.DisplayName;
    ...
    private static OrganizationSettingType GetSettingType(PropertyInfo propertyInfo)
    {
        switch (Type.GetTypeCode(propertyInfo.PropertyType))
        {
            ...
            case TypeCode.Boolean: return OrganizationSettingType.Boolean;

            default:
                if (propertyInfo.GetCustomAttribute<EmailAddressAttribute>() != null) return OrganizationSettingType.Email;

                if (propertyInfo.GetCustomAttribute<UrlAttribute>() != null) return OrganizationSettingType.Url;
    ...
    private static string NormalizeKey(string section, string name) => section.Trim().ToLower() + "." + name.Trim().ToLower();
}