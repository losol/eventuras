...
public class OrganizationSettingsAccessorService : IOrganizationSettingsAccessorService
{
    ...
    public async Task<T> ReadOrganizationSettingsAsync<T>()
    {
        ...
        var settingsMap = settings.ToDictionary(s => s.Name, s => s.Value);
        ...
        if (poco is not IConfigurableSettings { Enabled: false }) Validator.ValidateObject(poco, new ValidationContext(poco, null, null), true);
