...
public class OrganizationSettingsCache : IOrganizationSettingsCache
{
    ...
    {
        return await _memoryCache.GetOrCreateAsync(KeyForOrg(organizationId),
            async _ => { return await _context.OrganizationSettings.AsNoTracking().Where(s => s.OrganizationId == organizationId).ToArrayAsync(); });
    }
