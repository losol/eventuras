...
public class OrganizationSettingsManagementService : IOrganizationSettingsManagementService
{
    ...
    public async Task CreateOrganizationSettingAsync(OrganizationSetting setting, CancellationToken cancellationToken = default)
    {
        if (setting == null) throw new ArgumentNullException(nameof(setting));

        await _organizationAccessControlService.CheckOrganizationUpdateAccessAsync(setting.OrganizationId);

        if (await _context.OrganizationSettings.AnyAsync(s => s.OrganizationId == setting.OrganizationId && s.Name == setting.Name,
                cancellationToken))
            throw new DuplicateException($"Setting {setting.Name} already exists for organization {setting.OrganizationId}");
        ...
        catch (DbUpdateException e) when (e.IsUniqueKeyViolation())
        {
            ...
            throw new DuplicateException($"Setting {setting.Name} already exists for organization {setting.OrganizationId}", e);
        }
    }

    public async Task UpdateOrganizationSettingAsync(OrganizationSetting setting, CancellationToken cancellationToken = default)
    {
        if (setting == null) throw new ArgumentNullException(nameof(setting));
    ...
    public async Task RemoveOrganizationSettingAsync(OrganizationSetting setting, CancellationToken cancellationToken = default)
    {
        if (setting == null) throw new ArgumentNullException(nameof(setting));
