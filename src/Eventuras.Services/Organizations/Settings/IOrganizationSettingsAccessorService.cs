...
{
    /// <summary> Retrieves the org-level setting value. </summary>
    ...
    /// <summary>
    /// Reads settings as a POCO. POCO properties could be annotated with <see cref="OrgSettingKeyAttribute" /> to map them to setting values. Otherwise POCO
    /// type name + property name will be used as a setting keys.
    /// </summary>
    ...
    Task<T> ReadOrganizationSettingsAsync<T>();
}