...
public class OrganizationAccessControlService : IOrganizationAccessControlService
{
    ...
    public OrganizationAccessControlService(ApplicationDbContext context, IHttpContextAccessor httpContextAccessor)
    {
    ...
    private async Task CheckAdminRoleAsync(int organizationId)
    {
        ...
        if (user.IsPowerAdmin()) return;

        var member = await _context.OrganizationMembers.AsNoTracking()
            .Include(m => m.Roles)
            .SingleOrDefaultAsync(m => m.OrganizationId == organizationId && m.UserId == user.GetUserId());

        if (member?.HasRole(Roles.Admin) != true) throw new NotAccessibleException($"User is not allowed to update organization {organizationId}");
    }
}