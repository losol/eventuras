...
internal class RegistrationManagementService : IRegistrationManagementService
{
        ...
        CancellationToken cancellationToken = default)
    {
        var existingRegistration =
            await _context.Registrations.FirstOrDefaultAsync(m => m.EventInfoId == eventId && m.UserId == userId, cancellationToken);

        if (existingRegistration != null) throw new DuplicateException("Found existing registration for user on event.");
        ...
        if (eventInfo.Status == EventInfo.EventInfoStatus.WaitingList) registration.Status = Registration.RegistrationStatus.WaitingList;
        ...
        {
            var mandatoryItems = eventInfo.Products.Where(p => p.IsMandatory)
                .Select(p => new OrderLineModel(p.ProductId, null, p.MinimumQuantity))
                .ToArray();

            if (mandatoryItems.Any())
                await _orderManagementService.CreateOrderForRegistrationAsync(registration.RegistrationId, mandatoryItems, cancellationToken);
        }

        if (eventInfo.MaxParticipants > 0 && eventInfo.Registrations.Count + 1 >= eventInfo.MaxParticipants)
        {
    ...
    public async Task UpdateRegistrationAsync(Registration registration, CancellationToken cancellationToken = default)
    {
