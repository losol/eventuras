...
{
    /// <summary> Marks order as canceled (soft delete). </summary>
    /// <param name="order"> Order to cancel. </param>
    /// <param name="cancellationToken"> Cancellation token. </param>
    Task CancelOrderAsync(Order order, CancellationToken cancellationToken = default);

    /// <summary> Updates order lines in order. </summary>
    ...
    /// <summary> Creates a new order for given registration. </summary>
    ...
    /// <summary>
    /// Creates a new order or updates existing order to make registration have all specified <paramref name="expectedOrderLines" /> for all orders
    /// in total.
    /// </summary>
    /// <remarks>
    /// Order will be created only if there are no open orders (with status <see cref="Order.OrderStatus.Draft" />) in registration. If registration has
    /// multiple open orders, last one by creation date is selected for update.
    /// </remarks>
