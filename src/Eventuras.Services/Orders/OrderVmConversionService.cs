...
internal class OrderVmConversionService : IOrderVmConversionService
{
    ...
    public async Task<IEnumerable<OrderDTO>> OrderVmsToOrderDtos(ICollection<OrderVM> orders)
    {
        ...
        var productMap = await context.Products.Where(p => productIds.Contains(p.ProductId)).ToDictionaryAsync(p => p.ProductId);

        var variantMap = await context.ProductVariants.Where(v => variantIds.Contains(v.ProductVariantId)).ToDictionaryAsync(v => v.ProductVariantId);
    ...
    {
        var o = orders.FirstOrDefault(o => productMap[o.ProductId].MinimumQuantity > 0 && o.Quantity < productMap[o.ProductId].MinimumQuantity);
        if (o != null)
        {
    ...
    private static void CheckProductIds(IEnumerable<OrderVM> orders, IReadOnlyDictionary<int, Product> productMap)
    {
        ...
        if (o != null) throw new NotFoundException($"Product #{o.ProductId} not found");
    }

    private static void CheckVariantIds(IEnumerable<OrderVM> orders, IReadOnlyDictionary<int, ProductVariant> variantMap)
    {
        var o = orders.FirstOrDefault(o => o.VariantId.HasValue && !variantMap.ContainsKey(o.VariantId.Value));
        if (o != null) throw new NotFoundException($"Product variant #{o.VariantId} not found");
    }
}