...
public class OrderManagementService : IOrderManagementService
{
    ...
    public async Task UpdateOrderLinesAsync(Order order, ICollection<OrderLineModel> updatedOrderLines, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(order);
        ArgumentNullException.ThrowIfNull(updatedOrderLines);

        if (!order.CanEdit) throw new InvalidOperationServiceException($"Order {order.OrderId} cannot be updated being in {order.Status} status");
        ...
        {
            var existingOrderLine =
                order.OrderLines.FirstOrDefault(ol => ol.ProductId == line.ProductId && ol.ProductVariantId == line.ProductVariantId);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var registration = await GetRegistrationForUpdate(registrationId, cancellationToken);
        var registrationOrderLines = registration.Orders.SelectMany(o => o.OrderLines).Select(OrderLineModel.FromOrderLineDomainModel);
        ...
        static ICollection<OrderLineModel> GetDifferenceInOrderLines(ICollection<OrderLineModel> expected, ICollection<OrderLineModel> actual)
        {
            var toAdd = expected.ExceptBy(actual.Select(ol => new { ol.ProductId, ol.ProductVariantId }),
            ...
            return toAdd.Union(toRemove).Union(toUpdate).Where(ol => ol.Quantity != 0).ToArray();

            static IEnumerable<OrderLineModel> GetUpdates(ICollection<OrderLineModel> expected, IEnumerable<OrderLineModel> actual)
            {
                var intersect = actual.IntersectBy(expected.Select(ol => new { ol.ProductId, ol.ProductVariantId }),
                    ol => new { ol.ProductId, ol.ProductVariantId });
    ...
    private async Task<OrderLine> CreateOrderLine(EventInfo eventInfo, OrderLineModel line, CancellationToken cancellationToken)
    {
        ...
        catch (NotFoundException e) { throw new ArgumentServiceException($"Product with id {line.ProductId} was not found", null, e); }
    ...
    private async Task ValidateMinimumQuantityForProducts(Registration registration, CancellationToken cancellationToken = default)
    {
        ...
        var orderedProductsWithAllVariants = registrationOrderLines.GroupBy(ol => ol.ProductId)
        ...
        async Task LoadProductsInEventInfoWithMinimumAmount(EventInfo ei)
        {
            ...
            {
                var query = collectionNavigation.Query().Where(p => p.MinimumQuantity > 0);
    ...
    private async Task ValidateProductVisibility(Product product, EventInfo orderEventInfo, CancellationToken cancellationToken = default)
    {
        if (product.Visibility == ProductVisibility.Collection)
        {
            ...
            var commonCollections =
                orderEventInfo.Collections.IntersectBy(product.EventInfo.Collections.Select(c => c.CollectionId), ec => ec.CollectionId);
        ...
        {
            throw new ArgumentServiceException($"Product with id {product.ProductId} is not visible for event with id {orderEventInfo.EventInfoId}");
        }
