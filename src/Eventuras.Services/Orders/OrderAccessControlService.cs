...
public class OrderAccessControlService : IOrderAccessControlService
{
    ...
    public async Task CheckOrderReadAccessAsync(Order order, CancellationToken cancellationToken)
    {
    ...
    public async Task<IQueryable<Order>> AddAccessFilterAsync(IQueryable<Order> query, CancellationToken cancellationToken = default)
    {
        var user = _httpContextAccessor.HttpContext.User;
        if (user.IsAnonymous()) throw new NotAccessibleException("Anonymous users are not permitted to list any orders.");

        if (user.IsPowerAdmin()) return query;

        var userId = user.GetUserId();
        if (!user.IsAdmin())
            // non-admins can only read their own orders
            return query.Where(o => o.UserId == userId || o.Registration.UserId == userId);
    ...
    private async Task<Registration> GetOrderRegistrationAsync(Order order, CancellationToken cancellationToken)
        => order.Registration
        ?? await _registrationRetrievalService.GetRegistrationByIdAsync(order.RegistrationId, cancellationToken: cancellationToken);
}