...
internal class OrderService : IOrderService
{
    ...
    public async Task<List<Order>> GetAsync()
        => await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).OrderByDescending(o => o.OrderTime).ToListAsync();

    public async Task<List<Order>> GetWithRegistrationsAsync()
        => await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Include(o => o.Registration)
            .ThenInclude(r => r.User)
            .OrderByDescending(o => o.OrderTime)
            .ToListAsync();

    public async Task<List<Order>> GetAsync(int count, int offset)
        => await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Skip(offset).Take(count).OrderByDescending(o => o.OrderTime).ToListAsync();

    public async Task<List<Order>> GetAsync(int count) => await GetAsync(count, 0);

    public async Task<Order> GetByIdAsync(int orderId)
        => await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Where(o => o.OrderId == orderId)
    ...
    public async Task<List<Order>> GetOrdersForEventAsync(int eventId)
        => await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Include(o => o.OrderLines)
            .Include(o => o.Registration)
    ...
    {
        if (order.Status == OrderStatus.Invoiced) throw new InvalidOperationException("Invoiced orders cannot be deleted.");
    ...
    public Task<OrderLine> GetOrderLineAsync(int lineId)
        => _db.OrderLines.Where(l => l.OrderLineId == lineId).Include(l => l.Order).SingleOrDefaultAsync();

    public async Task<bool> DeleteOrderLineAsync(int lineId)
    {
        var line = await GetOrderLineAsync(lineId);
        if (line == null) throw new ArgumentException("Invalid lineId", nameof(lineId));

        if (!line.Order.CanEdit) throw new InvalidOperationException("The order line cannot be edited anymore.");
    ...
    public async Task<bool> UpdateOrderLine(int lineId, int? variantId, int quantity, decimal price)
    {
        ...
        if (variantId != null)
        {
            var variant = await _db.ProductVariants.AsNoTracking().SingleOrDefaultAsync(v => v.ProductVariantId == variantId);
    ...
    {
        var order = await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Include(o => o.OrderLines).SingleOrDefaultAsync(o => o.OrderId == id);
    ...
    {
        var order = await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Where(m => m.OrderId == orderId).FirstOrDefaultAsync();
    ...
    {
        var order = await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Where(m => m.OrderId == orderId).SingleOrDefaultAsync();
    ...
    {
        var order = await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).Include(o => o.Registration)
        ...
        var existingProductIds = order.Registration.Orders.Where(o => o.Status != OrderStatus.Cancelled)
            .SelectMany(o => o.OrderLines.Where(l => l.ProductId.HasValue).Select(l => l.ProductId.Value));
    ...
    private async Task<IQueryable<Order>> AddCurrentOrgFilterForAdminAsync(IQueryable<Order> query)
    {
        ...
        if (principal.IsInRole(Roles.SuperAdmin)) return query;

        if (principal.Identity.IsAuthenticated)
        {
            var organization = await _currentOrganizationAccessorService.RequireCurrentOrganizationAsync();
            return organization?.IsRoot == true ? query : query.HavingOrganization(organization);
        }
        else
        {
    ...
    {
        var order = await (await AddCurrentOrgFilterForAdminAsync(_db.Orders)).SingleOrDefaultAsync(o => o.OrderId == orderId);

        if (order == null) throw new InvalidOperationException($"Order {orderId} doesn't exist or not accessible");
