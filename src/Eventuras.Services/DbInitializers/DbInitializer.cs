...
public class DbInitializer : IDbInitializer
{
    ...
    {
        if (runMigrations) await _db.Database.MigrateAsync();
        ...
        foreach (var roleName in roleNames)
        {
            ...
            if (!roleExist) roleResult = await _roleManager.CreateAsync(new IdentityRole(roleName));
        }
        ...
        if (createSuperUser)
            ...
            if (!_userManager.GetUsersInRoleAsync(Roles.SuperAdmin).Result.Any())
            {
                ...
                if (user == null)
                {
                    ...
                    var UserPassword = _config.SuperAdmin.Password;
                    var createSuperAdmin = await _userManager.CreateAsync(superadmin, UserPassword);
                    if (createSuperAdmin.Succeeded) await _userManager.AddToRoleAsync(superadmin, Roles.SuperAdmin);
                }
            }
