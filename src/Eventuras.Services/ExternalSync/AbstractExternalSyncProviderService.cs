...
/// <summary>
/// External event synchronization stub. Common approach to external synchronization is to make checks whether the external event is registered in the
/// system, whether the external account exists and create one if it doesn't. All external system interactions are delegated to the descendant classes.
/// </summary>
public abstract class AbstractExternalSyncProviderService : IExternalSyncProviderService
{
    ...
    public virtual async Task<ExternalEventSyncResult> RunSynchronizationAsync(ExternalAccount account, Registration registration)
    {
        ...
        if (registration.EventInfo == null) throw new ArgumentException(nameof(registration));

        var externalEvent = await EnsureExternalEventAsync(registration.EventInfo);
        if (externalEvent == null) return ExternalEventSyncResult.NotSynced;

        if (await _context.ExternalRegistrations.AnyAsync(e => e.ExternalEventId == externalEvent.LocalId && e.ExternalAccountId == account.LocalId))
        ...
        try { await _context.CreateAsync(externalRegistration); }
    ...
    public async Task<ExternalAccount> CreateAccountForUserAsync(Registration registration)
    {
        ...
        if (!registration.Verified) throw new InvalidOperationException($"Registration {registration.RegistrationId} is not verified");
    ...
    /// <summary> Implementation may decide to perform some actions in addition to just creating external account. Default is do nothing. </summary>
    ...
    /// <summary>
    /// Creates new <see cref="ExternalAccount" /> for the given registration. Implementation may decide to perform additional checks whether the account is
    /// already exists in the external service and re-use the existing data or create new record.
    /// </summary>
    ...
    /// <summary>
    /// Checks whether the external event setup is properly done for the given event info and returns the corresponding external event object or throws
    /// <see cref="ExternalSyncException" /> in case of any issues.
    /// </summary>
    ...
    protected virtual async Task<ExternalEvent> EnsureExternalEventAsync(EventInfo eventInfo)
    {
        var externalEvent = await _context.ExternalEvents.FirstOrDefaultAsync(
            c => c.ExternalServiceName == Name && c.EventInfoId == eventInfo.EventInfoId);

        if (externalEvent == null) throw new ExternalEventNotFoundException($"No external events configured for event {eventInfo.Title}");
