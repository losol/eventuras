...
public interface IExternalEventManagementService
{
    ...
    /// External event with the given <code>serviceProviderName</code> and <code>externalEventId</code> is
    /// already registered.
    /// </exception>
    Task<ExternalEvent> CreateNewExternalEventAsync(int eventInfoId, string externalServiceName, string externalEventId);
