...
/// <summary>
/// Manages one external account per single event registration. Which means, that every user registered in the system may have multiple external accounts
...
public abstract class AbstractExternalAccountPerRegistrationSyncProviderService : AbstractExternalSyncProviderService
{
    ...
    public override async Task<ExternalAccount> FindExistingAccountAsync(Registration registration)
    {
        ...
        return await _context.ExternalAccounts.FirstOrDefaultAsync(a
            => a.ExternalServiceName == Name && a.RegistrationId == registration.RegistrationId);
    }
}