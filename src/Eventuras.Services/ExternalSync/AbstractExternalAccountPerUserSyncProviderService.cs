...
/// <summary> Manages one external account per user, in contrast with having one external account per registration. </summary>
public abstract class AbstractExternalAccountPerUserSyncProviderService : AbstractExternalSyncProviderService
{
    ...
    public override async Task<ExternalAccount> FindExistingAccountAsync(Registration registration)
    {
        ...
        return await _context.ExternalAccounts.FirstOrDefaultAsync(a => a.ExternalServiceName == Name && a.UserId == registration.UserId);
    }
}