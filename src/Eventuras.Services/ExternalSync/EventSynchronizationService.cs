...
internal class EventSynchronizationService : IEventSynchronizationService
{
    ...
    public async Task<EventSynchronizationResult[]> SyncEvent(int eventId, string syncProviderName, CancellationToken cancellationToken)
    {
    ...
    private async Task<EventSynchronizationResult> SyncAllRegistrationsAsync(
        IExternalSyncProviderService syncProviderService,
        EventInfo eventInfo,
        CancellationToken cancellationToken)
    {
        var result = new EventSynchronizationResult(syncProviderService.Name);

        var reader = new PageReader<Registration>(async (offset, limit, token) => await _registrationRetrievalService.ListRegistrationsAsync(
            ...
            RegistrationRetrievalOptions.UserAndEvent,
            token));
        ...
        while (await reader.HasMoreAsync(cancellationToken))
            foreach (var registration in await reader.ReadNextAsync(cancellationToken))
            {
                ...
                if (externalAccount != null) await SyncRegistrationAsync(syncProviderService, registration, externalAccount, result);
            }

        return result;
    }

    private async Task<List<IExternalSyncProviderService>> FilterProvidersAsync(
        EventInfo eventInfo,
        string syncProviderName,
        List<EventSynchronizationResult> results)
    {
        var filteredServices = new List<IExternalSyncProviderService>();
        foreach (var syncProviderService in _syncProviderServices)
        {
            if (!string.IsNullOrEmpty(syncProviderName) && !string.Equals(syncProviderName, syncProviderService.Name)) continue;
            ...
            catch (ExternalSyncException e) { results.Add(new EventSynchronizationResult(syncProviderService.Name).AddGenericError(e)); }
        }

        return filteredServices;
    }

    private async Task<ExternalAccount> CreateExternalAccountIfNotExists(
        IExternalSyncProviderService service,
        Registration registration,
        EventSynchronizationResult result)
    {
        try
        {
            _logger.LogInformation("Looking for external {serviceName} account for registration [{registrationId}], user [{userId}]",
                service.Name,
    ...
    private async Task SyncRegistrationAsync(
        IExternalSyncProviderService service,
        Registration registration,
        ...
        EventSynchronizationResult result)
    {
        ...
        {
            _logger.LogWarning("Event info [{eventInfoId}] ({eventInfoTitle}) has no associated {} external event id. Skipping registration sync.",
                registration.EventInfo.EventInfoId,
                registration.EventInfo.Title,
                service.Name);
        }
        catch (Exception e) { result.UserExportErrors.Add(registration.UserId, e); }
    }
}