...
internal class NotificationManagementService : INotificationManagementService
{
    ...
    public async Task<EmailNotification> CreateEmailNotificationAsync(string subject, string body, string[] recipients)
    {
        ...
        return await _context.CreateAsync(new EmailNotification(subject, body)
            {
                ...
                Recipients = recipients.Select(NotificationRecipient.Email).ToList(),
            },
            true);
    }
        ...
        Registration.RegistrationType[] registrationTypes)
    {
        ...
        var recipients = await GetRecipientsAsync(NotificationType.Email, eventId, productId, registrationStatuses, registrationTypes);
    ...
    public async Task<SmsNotification> CreateSmsNotificationAsync(string message, params string[] recipients)
    {
        var currentOrg = await _currentOrganizationAccessorService.GetCurrentOrganizationAsync();

        var currentUser = _httpContextAccessor.HttpContext.User;

        return await _context.CreateAsync(new SmsNotification(message)
            {
                CreatedByUserId = currentUser.GetUserId(),
                OrganizationId = currentOrg?.OrganizationId,
                Recipients = recipients.Select(NotificationRecipient.Sms).ToList(),
            },
            true);
    }
        ...
        Registration.RegistrationType[] registrationTypes = null)
    {
        ...
        var recipients = await GetRecipientsAsync(NotificationType.Sms, eventId, productId, registrationStatuses, registrationTypes);
    ...
    private async Task CheckEventAccessAsync(int eventId)
    {
        ...
        await _eventInfoAccessControlService.CheckEventManageAccessAsync(eventInfo);
    }
    ...
    private static void CheckSubjectAndBody(string subject, string body)
    {
        ...
        if (string.IsNullOrEmpty(body)) throw new ArgumentException($"{nameof(body)} must not be empty");
    }
        ...
        Registration.RegistrationType[] registrationTypes)
    {
        ...
        var reader = new PageReader<Registration>(async (offset, limit, token) => await _registrationRetrievalService.ListRegistrationsAsync(
            new RegistrationListRequest
            {
                ...
                {
                    EventInfoId = eventId,
                    ProductIds = productId.HasValue ? new[] { productId.Value } : Array.Empty<int>(),
        ...
        while (await reader.HasMoreAsync())
            recipients.AddRange((
                from registration in await reader.ReadNextAsync()
                select NotificationRecipient.Create(registration, notificationType)).Where(r => r != null)); // user may have no phone, or email
    ...
    public async Task UpdateNotificationAsync(Notification notification)
    {
        ...
        if (notification.EventInfoId.HasValue) await CheckEventAccessAsync(notification.EventInfoId.Value);
