...
public class CertificateIssuingService : ICertificateIssuingService
{
    ...
    public async Task<ICollection<Certificate>> CreateCertificatesForEventAsync(EventInfo eventInfo, CancellationToken cancellationToken)
    {
        await _eventInfoAccessControlService.CheckEventManageAccessAsync(eventInfo, cancellationToken);

        var certificates = new List<Certificate>();

        for (var reader = ReadRegistrations(eventInfo, havingNoCertificateOnly: true); await reader.HasMoreAsync(cancellationToken);)
        {
            certificates.AddRange(
                from registration in await reader.ReadNextAsync(cancellationToken)
    ...
    public async Task<ICollection<Certificate>> UpdateCertificatesForEventAsync(EventInfo eventInfo, CancellationToken cancellationToken)
    {
        ...
        for (var reader = ReadRegistrations(eventInfo, true); await reader.HasMoreAsync(cancellationToken);)
        {
            certificates.AddRange(
                from registration in await reader.ReadNextAsync(cancellationToken)
    ...
    private PageReader<Registration> ReadRegistrations(EventInfo eventInfo, bool havingCertificateOnly = false, bool havingNoCertificateOnly = false)
    {
        return new PageReader<Registration>(async (offset, limit, token) => await _registrationRetrievalService.ListRegistrationsAsync(
            ...
            RegistrationRetrievalOptions.ForCertificateIssuing,
            token));
    }
}