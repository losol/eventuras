...
public class PageReader<T>
{
    ...
    public PageReader(Func< /*offset*/int, /*limit*/int, CancellationToken, Task<Paging<T>>> func, int count = PagingRequest.MaxRecordsPerPage)
    {
        if (count > PagingRequest.MaxRecordsPerPage)
            throw new ArgumentOutOfRangeException(nameof(count),
                $"{nameof(count)} must be not more than " + $"{nameof(PagingRequest)}.{nameof(PagingRequest.MaxRecordsPerPage)}");
    ...
    {
        await EnsureFetchedAsync(cancellationToken);
        if (_lastPageRead) throw new InvalidOperationException("Iteration is done");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (!cancellationToken.IsCancellationRequested && await reader.HasMoreAsync(cancellationToken))
