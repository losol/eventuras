...
public class RegistrationService : IRegistrationService
{
    ...
    {
        return await _db.Registrations.Include(m => m.User).Include(m => m.EventInfo).OrderByDescending(m => m.RegistrationId).Take(20).ToListAsync();
    }

    public async Task<Registration> GetAsync(int id) => await _db.Registrations.FindAsync(id);

    public async Task<Registration> GetAsync(string userId, int eventId)
    {
        var registration = await _db.Registrations.Where(a => a.UserId == userId && a.EventInfoId == eventId).FirstOrDefaultAsync();
        return registration;
    }

    public Task<Registration> GetWithOrdersAsync(int id)
    {
        return _db.Registrations.Where(r => r.RegistrationId == id).Include(r => r.Orders).ThenInclude(o => o.OrderLines).SingleOrDefaultAsync();
    }

    public async Task<Registration> GetWithUserAndEventInfoAsync(int id)
    {
        return await _db.Registrations.Where(x => x.RegistrationId == id).Include(r => r.EventInfo).SingleOrDefaultAsync();
    }

    public async Task<Registration> GetWithUserAndEventInfoAndOrders(int id)
        => await _db.Registrations.Include(r => r.Orders)
    ...
    {
        return await _db.Registrations.Where(r => r.EventInfoId == eventId && r.Status != RegistrationStatus.Cancelled)
            .Include(r => r.EventInfo)
            .Include(r => r.User)
            .ToListAsync();
    }
    ...
    public async Task<List<Registration>> GetRegistrationsWithOrders(int eventId)
        => await _db.Registrations.Where(r => r.EventInfoId == eventId)
            .Include(r => r.Orders)
            .ThenInclude(o => o.OrderLines)
            .Include(r => r.User)
            .ToListAsync();
    ...
    {
        var reg = await _db.Registrations.Where(m => m.UserId == user.Id)
    ...
    public async Task<int> CreateRegistration(Registration registration, List<OrderVM> ordersVm)
    {
        ...
        if (existingRegistration != null) throw new InvalidOperationException("The user can only register once!");
    ...
    public Task<int> CreateRegistration(Registration registration) => CreateRegistration(registration, null);
    ...
    {
        var userId = await _db.Users.Where(u => u.Email == email).Select(u => u.Id).SingleOrDefaultAsync();
        _ = userId ?? throw new ArgumentException("Invalid email.", nameof(email));

        var registration = await _db.Registrations.Where(a => a.UserId == userId && a.EventInfoId == eventId)
            .Include(r => r.Orders)
            .ThenInclude(o => o.OrderLines)
            .SingleOrDefaultAsync();
        _ = registration ?? throw new ArgumentException("Invalid eventId or the user is not registered for the event", nameof(eventId));
    ...
    {
        var registration = await _db.Registrations.Where(a => a.RegistrationId == registrationId)
            .Include(r => r.Orders)
            .ThenInclude(o => o.OrderLines)
            ...
            .SingleOrDefaultAsync();
        _ = registration ?? throw new ArgumentException("Invalid registration id.", nameof(registrationId));
    ...
    {
        var reg = await _db.Registrations.Where(m => m.RegistrationId == registrationId).FirstOrDefaultAsync();
    ...
    public async Task<bool> UpdateCustomerInfo(
        int registrationId,
        string customerName,
        string customerEmail,
        string customerVatNumber,
        string customerInvoiceReference)
    {
        var reg = await _db.Registrations.Where(m => m.RegistrationId == registrationId).Include(m => m.Orders).FirstOrDefaultAsync();
        ...
        foreach (var order in reg.Orders.Where(s => s.CanEdit))
        {
    ...
    public async Task<bool> UpdateCustomerAddress(
        int registrationId,
    ...
    public async Task<bool> UpdatePaymentMethod(int registrationId, PaymentProvider paymentMethod)
    {
        ...
        foreach (var order in reg.Orders.Where(s => s.CanEdit)) order.PaymentMethod = paymentMethod;

        return await _db.SaveChangesAsync() > 0;
    }

    public async Task<bool> UpdateRegistrationStatus(int registrationId, RegistrationStatus status)
    {
        var reg = await _db.Registrations.Where(m => m.RegistrationId == registrationId).FirstOrDefaultAsync();
    ...
    public async Task<bool> UpdateRegistrationType(int registrationId, RegistrationType type)
    {
        var reg = await _db.Registrations.Where(m => m.RegistrationId == registrationId).FirstOrDefaultAsync();
