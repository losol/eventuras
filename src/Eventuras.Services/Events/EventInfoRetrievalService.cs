...
internal class EventInfoRetrievalService : IEventInfoRetrievalService
{
    ...
    public async Task<EventInfo> GetEventInfoByIdAsync(int id, EventInfoRetrievalOptions options, CancellationToken cancellationToken)
    {
        return await _context.EventInfos.UseOptions(options ?? new EventInfoRetrievalOptions())
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Filter.AccessibleOnly) query = await AddOrgFilterIfNeededAsync(query, cancellationToken);

        return await Paging<EventInfo>.CreateAsync(query, request, cancellationToken);
    }

    private async Task<IQueryable<EventInfo>> AddOrgFilterIfNeededAsync(IQueryable<EventInfo> query, CancellationToken cancellationToken)
    {
        var principal = _httpContextAccessor.HttpContext.User;
        if (principal.IsAnonymous())
        {
            var organization = await _currentOrganizationAccessorService.GetCurrentOrganizationAsync(null, cancellationToken);

            return organization?.IsRoot == true ? query : query.HavingNoOrganizationOr(organization);
        }
