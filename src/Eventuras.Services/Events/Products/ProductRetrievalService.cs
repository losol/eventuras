...
internal class ProductRetrievalService : IProductRetrievalService
{
    ...
    public async Task<ICollection<Product>> ListProductsAsync(
        ...
        => await _context.Products.UseFilter(request.EventInfoId, request.Filter)
            .UseOrder(request.Order, request.Descending)
            .UseOptions(options ?? ProductRetrievalOptions.Default)
            .ToArrayAsync(cancellationToken);

    public async Task<Product> GetProductByIdAsync(int productId, ProductRetrievalOptions options, CancellationToken cancellationToken)
    {
        return await _context.Products.UseOptions(options ?? ProductRetrievalOptions.Default)
