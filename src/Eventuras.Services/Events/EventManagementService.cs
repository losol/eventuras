...
internal class EventManagementService : IEventManagementService
{
    ...
    public EventManagementService(ApplicationDbContext context, IProductsService productsService)
    {
    ...
    public async Task CreateNewEventAsync(EventInfo info)
    {
        ...
        if (await _context.EventInfos.AnyAsync(e => e.Slug == info.Slug)) throw new DuplicateException($"Event with code {info.Slug} already exists");

        try { await _context.CreateAsync(info); }
    ...
    public async Task UpdateEventAsync(EventInfo info)
    {
        ...
        if (info.Products != null)
        {
            ...
            var providedVariants = info.Products.Where(p => p.ProductVariants != null).SelectMany(p => p.ProductVariants);

            var variantsToDelete = originalVariants.Where(originalVariant
                => providedVariants.All(variant => variant.ProductVariantId != originalVariant.ProductVariantId));

            _context.ProductVariants.RemoveRange(variantsToDelete);

            // Delete the products that don't exist in the provided object
            var productsToDelete = originalProducts.Where(op => info.Products.All(p => p.ProductId != op.ProductId));

            _context.Products.RemoveRange(productsToDelete);
            await _context.SaveChangesAsync();
        }

        if (await _context.EventInfos.AnyAsync(e => e.Slug == info.Slug && e.EventInfoId != info.EventInfoId && !e.Archived))
            throw new DuplicateException($"Event with code {info.Slug} already exists");

        try { await _context.UpdateAsync(info); }
        catch (DbUpdateException e) when (e.IsUniqueKeyViolation())
        {
