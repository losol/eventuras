...
internal class EventProductsManagementService : IEventProductsManagementService
{
    ...
    public async Task UpdateEventProductsAsync(int eventId, List<Product> products)
    {
        ...
        var originalProducts = await _context.Products.Where(p => p.EventInfoId == eventId)
        ...
        var providedVariants = products.Where(p => p.ProductVariants != null).SelectMany(p => p.ProductVariants);
        ...
        // Save the updates
        var info = await _context.EventInfos.Where(e => e.EventInfoId == eventId)
