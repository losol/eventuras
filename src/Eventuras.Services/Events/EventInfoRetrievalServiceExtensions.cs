...
{
    /// <summary> Shortcut for <see cref="IEventInfoRetrievalService.GetEventInfoByIdAsync" />. </summary>
    ...
    public static async Task<List<EventInfo>> GetAllEventsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return (await PageReader<EventInfo>.ReadAllAsync((offset, limit, token) => service.ListEventsAsync(new EventListRequest(offset, limit)
                {
                    Filter = filter,
                },
                options,
