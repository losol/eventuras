...
public class NotificationRecipient
{
    ...
    /// <summary> Optional reference to the recipient application user. </summary>
    public string RecipientUserId { get; set; }

    /// <summary> Optional event registration reference. </summary>
    public int? RegistrationId { get; set; }

    public string RecipientName { get; private set; }

    /// <summary> The phone number or email address which the notification is sent to. </summary>
    [Required]
    ...
    {
        if (recipientIdentifier == null) throw new ArgumentNullException(nameof(recipientIdentifier));
        ...
        else { RecipientIdentifier = recipientIdentifier; }
    }
    ...
    public static NotificationRecipient Create(Registration registration, NotificationType notificationType)
    {
        ...
        if (recipient != null) recipient.RecipientName = registration.ParticipantName ?? registration.User.Name;
    ...
    public static NotificationRecipient Email(Registration registration) => Create(registration, NotificationType.Email);

    public static NotificationRecipient Sms(string phoneNumber)
    {
        if (string.IsNullOrEmpty(phoneNumber)) throw new ArgumentException($"{nameof(phoneNumber)} must not be empty");

        return new NotificationRecipient(phoneNumber, NotificationType.Sms);
    }

    public static NotificationRecipient Sms(ApplicationUser recipientUser) => Create(recipientUser, NotificationType.Sms);
