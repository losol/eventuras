...
public class Registration
{
    ...
    [NotMapped]
    private IEnumerable<string> NameParts => ParticipantName?.Split(" ")?.Select(p => p.Trim());
    ...
    public PaymentProvider PaymentMethod { get; set; } =
        PaymentProvider.PowerOfficeEmailInvoice; // HACK: This ignores the actual default paymentmethod set in the database
    ...
    public List<OrderDTO> GetCurrentProducts() => _getProductsForOrders(Orders.Where(o => o.Status != OrderStatus.Cancelled));

    public List<OrderDTO> GetInvoicedProducts() => _getProductsForOrders(Orders.Where(o => o.Status is OrderStatus.Invoiced or OrderStatus.Refunded));
    ...
    {
        if (Certificate != null)
            throw new InvalidOperationException($"{nameof(Registration)}.{nameof(Certificate)} is already set for registration {RegistrationId}");
    ...
    {
        if (Certificate == null) throw new InvalidOperationException($"{nameof(Registration)}.{nameof(Certificate)} is null");

        if (EventInfo == null) throw new InvalidOperationException($"{nameof(Registration)}.{nameof(EventInfo)} is null");

        if (User == null) throw new InvalidOperationException($"{nameof(Registration)}.{nameof(User)} is null");
    ...
    public Order CreateOrder(IEnumerable<OrderDTO> orders, IEnumerable<OrderLine> refundlines = null)
    {
        ...
        if (orders != null && orders.Where(p => p.Product.EventInfoId != EventInfoId).Any())
            throw new ArgumentException("All the products must belong to the event being registered for.", nameof(orders));
        ...
        if (!orders.Where(o => o.Variant != null).All(o => o.Variant.ProductId == o.Product.ProductId))
            throw new ArgumentException("Variant & Product IDs must match.", nameof(orders));
    ...
    /// <summary> Updates an existing order if it's not already been invoiced. Else creates a new order. </summary>
    ...
    public Order CreateOrUpdateOrder(ICollection<OrderDTO> dtos)
    {
        ...
        if (!editableOrderExists)
        {
            ...
            foreach (var p in products)
                if (order.Product.ProductId == p.Product.ProductId)
                {
                    ...
                    if (orderline.Quantity != 0) refundLines.Add(orderline);

                    if (!shouldReplaceProductVariant) refundDtos.Add(order);
                }
        ...
        // an editable (uninvoiced) order exists
        {
    ...
    public void UpdateOrder(Order orderToUpdate, IEnumerable<OrderDTO> orders)
    {
        if (orderToUpdate.RegistrationId != RegistrationId)
            throw new InvalidOperationException($"Order {orderToUpdate.OrderId} doesn't belong to registration {RegistrationId}");
        ...
        foreach (var order in orders)
        {
            ...
            if (match != null) lines.Remove(match);
            ...
            else
            {
                ...
                if (orderline.Quantity != 0) lines.Add(orderline);
            }
        }
    }

    private List<OrderLine> _createOrderLines(IEnumerable<OrderDTO> orders, IEnumerable<OrderLine> refundlines = null)
    {
...
/// <summary> Compares OrderDTOs using the ProductId & VariantId </summary>
...
public static class OrderDTOExtensions
{
    ...
    {
        return orders.Where(o => o.Quantity != 0).Select(o => o.ToOrderLine()).ToList();
    }
