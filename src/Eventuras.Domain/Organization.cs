...
public class Organization
{
    ...
    [NotMapped]
    public string NameAndHostname => Hostnames?.Any(h => h.Active) == true ? $"{Name} ({CommaSeparatedHostnames})" : Name;

    [NotMapped]
    [DisplayName("Kommaseparerte Eventuras vertsnavn")]
    public string CommaSeparatedHostnames => string.Join(",", Hostnames?.Where(h => h.Active).Select(h => h.Hostname).ToList() ?? new List<string>());
