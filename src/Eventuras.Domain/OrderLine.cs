...
public class OrderLine
{
    ...
    /// <summary> A string that uniquely identifies a product-variant combination </summary>
    public string ItemCode => ProductVariantId.HasValue ? $"K{ProductId}-{ProductVariantId}" : $"K{ProductId}";

    /// <summary> A string which combines product and productvariant name </summary>
    public string ItemName => !string.IsNullOrWhiteSpace(ProductVariantName) ? $"{ProductName} ({ProductVariantName})" : $"{ProductName}";
    ...
    public OrderLine(Product product, int quantity, ProductVariant variant = null)
    {
        ...
        if (quantity == 0) throw new ArgumentException($"{nameof(quantity)} must be a non-zero value");

        if (variant != null && variant.ProductId != product.ProductId)
            throw new ArgumentException($"Product variant {variant.ProductVariantId} doesn't belong to product {product.ProductId}");
    ...
    {
        if (IsRefund) throw new InvalidOperationException("Cannot create a refund orderline for a refund orderline.");
