...
internal class ZoomSyncProviderService : AbstractExternalAccountPerRegistrationSyncProviderService
{
    ...
    public ZoomSyncProviderService(
        ApplicationDbContext context,
        IZoomApiClient zoomApiClient,
        ILogger<ZoomSyncProviderService> logger) : base(context, logger)
    {
    ...
    protected override async Task<ExternalEvent> EnsureExternalEventAsync(EventInfo eventInfo)
    {
        ...
        if (!externalEvent.TryGetZoomMeetingId(out _)) throw new InvalidZoomMeetingIdException(externalEvent.ExternalEventId);
    ...
    protected override async Task<ExternalAccount> CreateNewExternalAccountForRegistrationAsync(Registration registration)
    {
        ...
        try
        {
            ...
            var registrantId = registrants.FirstOrDefault(r => string.Equals(r.Email.Trim().ToLower(), normalizedUserEmail))?.RegistrantId;
