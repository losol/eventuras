...
internal class ZoomCredentialsAccessor : IZoomCredentialsAccessor
{
    ...
    {
        var apiKey = await _organizationSettingsAccessorService.GetOrganizationSettingByNameAsync(ZoomConstants.ZoomApiKeySettingKey);

        var apiSecret = await _organizationSettingsAccessorService.GetOrganizationSettingByNameAsync(ZoomConstants.ZoomApiSecretSettingKey);
        ...
        if (!host.HasValue) throw new InvalidOperationException("Cannot determine host for request");
