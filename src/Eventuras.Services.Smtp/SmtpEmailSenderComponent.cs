...
internal class SmtpEmailSenderComponent : IConfigurableEmailSenderComponent
{
    ...
    {
        var settings = await _organizationSettingsAccessorService.ReadOrganizationSettingsAsync<OrganizationSmtpSettings>();

        if (!settings.Enabled) return null;

        var logger = _loggerFactory.CreateLogger<SmtpEmailSender>();

        return new SmtpEmailSender(Options.Create(settings.ToSmtpConfig()), _healthCheckStorage, logger);
    }
}