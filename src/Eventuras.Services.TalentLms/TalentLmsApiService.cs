...
namespace Eventuras.Services.TalentLms
{
    internal class TalentLmsApiService : ITalentLmsApiService
    {
        ...
        public TalentLmsApiService(
            IHttpClientFactory httpClientFactory,
            IOptions<TalentLmsSettings> options,
            ILogger<TalentLmsApiService> logger)
        {
        ...
        public async Task<TalentLmsUserDto> UserSignUpAsync(TalentLmsUserSignUpRequest request)
        {
            ...
            return await PostAsync<TalentLmsUserDto>("usersignup", new[]
            {
        ...
        public async Task<TalentLmsEnrollmentDto> EnrollUserToCourseAsync(TalentLmsEnrollmentDto request)
        {
            ...
            if (!TalentLmsEnrollmentDto.Learner.Equals(request.Role) &&
                !TalentLmsEnrollmentDto.Instructor.Equals(request.Role))
            {
                throw new ValidationException($"Invalid enrollment role: {request.Role}");
            }

            var response = await PostAsync<TalentLmsEnrollmentDto[]>("addusertocourse", new[]
            {
            ...
            if (!response.Any())
            {
                throw new TalentLmsException("Empty response returned by enrollment method");
            }
        ...
        public async Task DeleteUserAsync(string userId, string deletedByUserId)
        {
            ...
            {
                throw new ArgumentException(nameof(deletedByUserId));
            }

            await PostAsync<TalentLmsDeleteUserResponse>("deleteuser", new[]
            {
        ...
        public async Task<TalentLmsUserDto[]> ListAllUsersAsync()
        {
            return await GetAsync<TalentLmsUserDto[]>("users");
        }
        ...
        private async Task<T> ReadTalentLmsResponseAsync<T>(HttpResponseMessage response)
        {
            ...
            {
                _logger.LogError("{statusCode} status code returned by TalentLMS endpoint ({responseText})",
                    response.StatusCode, responseText);

                switch (response.StatusCode)
                {
                    case HttpStatusCode.BadRequest:
                        throw new TalentLmsException($"A required parameter is missing or an invalid type (e.g.a string) was supplied instead of an integer ({responseText})");

                    case HttpStatusCode.Unauthorized:
                        throw new TalentLmsException($"Invalid API key provided ({responseText})");
                    ...
                    case HttpStatusCode.NotFound:
                        throw new TalentLmsException($"The requested resource (e.g. user) does not exist ({responseText})");

                    case HttpStatusCode.InternalServerError:
                        throw new TalentLmsException($"Internal server error ({responseText})");

                    default:
                        throw new TalentLmsException($"Unrecognized server response ({responseText})");
                }
            }
            ...
            try
            {
                ...
                if (result == null)
                {
                    throw new TalentLmsException($"Failed to read TalentLMS response: {responseText}");
                }
            ...
            {
                throw new TalentLmsException($"Failed to read TalentLMS response: {responseText}", e);
            }
            catch (ValidationException e)
            {
                throw new TalentLmsException($"TalentLMS returned invalid response: {responseText}", e);
            }
        }
    }
}
