...
public class StripeInvoicingService : IInvoicingProvider
{
    ...
    public bool AcceptPaymentProvider(PaymentMethod.PaymentProvider provider) => provider is PaymentMethod.PaymentProvider.StripeInvoice;
    ...
    public async Task<InvoiceResult> CreateInvoiceAsync(InvoiceInfo info)
    {
        ...
        var createInvoiceOptions = new StripeInvoiceCreateOptions
        {
            ...
            DaysUntilDue = info.DueDate.HasValue ? (info.DueDate.Value - SystemClock.Instance.Today()).Days : 30,
            Description = string.Join(", ", info.Lines.Where(l => l.Type == InvoiceLineType.Text).Select(l => l.Description)),
        };
