...
/// <summary> Create Identity user after Auth0 Sign in if needed and fill in claims in HTTP context. </summary>
internal class OauthTicketReceivedHandler : IOauthTicketReceivedHandler
{
    ...
    public OauthTicketReceivedHandler(UserManager<ApplicationUser> userManager, IOptions<Auth0IntegrationSettings> options)
    {
    ...
    public async Task TicketReceivedAsync(TicketReceivedContext context)
    {
        ...
        var claims = principal.GetAllClaimsExcept(ClaimTypes.NameIdentifier, ClaimTypes.Name, ClaimTypes.Role).ToList();
