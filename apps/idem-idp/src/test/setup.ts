...
if (!testDbUrl) {
  throw new Error(
    'No database URL found. Set IDEM_DATABASE_URL or IDEM_TEST_DATABASE_URL environment variable.'
  );
}

process.env.IDEM_DATABASE_URL = testDbUrl;

// Don't override SESSION_SECRET and MASTER_KEY - use dev defaults from config.ts
