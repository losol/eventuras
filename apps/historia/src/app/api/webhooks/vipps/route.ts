 ...
 *
 * Uses pspReference from payload as unique event identifier for idempotency.
 *
 * Events handled:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Use pspReference as unique identifier for this webhook event
    const eventId = payload.pspReference;
    ...
    if (existingEvent.docs.length > 0) {
      logger.info({ eventId, pspReference: payload.pspReference }, 'Event already processed (duplicate)');
    ...
    logger.info(
      { eventId, pspReference: payload.pspReference, businessEventId: businessEvent.id },
    ...
    try {
      ...
      logger.info(
        { eventId, pspReference: payload.pspReference, duration: Date.now() - startTime },
        'Webhook processed successfully'
      );
    } catch (error) {
      // Log error but still return 200 to acknowledge receipt
      logger.error(
        { error, eventId, pspReference: payload.pspReference, businessEventId: businessEvent.id },
