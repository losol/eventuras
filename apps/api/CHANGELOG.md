...
- ca7342d: The OpenAPI document is now written during the build instead of being fetched from a running API. Regenerating it takes a `dotnet build` — no server, no database, no Aspire stack — and CI fails when the committed document does not match the API, so the two can no longer drift apart.
...
- f28f42e: Upgrade dotnet packages
