...
public class PurposeDecision
{
    ...
    // Scopes the decision to a single registration, for purposes where one answer
    // per organization is too coarse: appearing on a public participant list, or
    // sharing dietary requirements with a venue, are decisions people make per
    // event rather than once for everything. Null means the decision covers the
    // whole organization.
    public Guid? RegistrationUuid { get; init; }
