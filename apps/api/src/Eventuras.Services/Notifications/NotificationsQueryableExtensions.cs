...
{
    public static IQueryable<Notification> AddFilter(
        this IQueryable<Notification> query,
        NotificationFilter filter)
    {
        if (filter.EventId.HasValue)
        {
            query = query.Where(n => n.EventInfoId == filter.EventId);
        }

        if (filter.ProductId.HasValue)
        {
            query = query.Where(n => n.ProductId == filter.ProductId);
        }
    ...
    public static IQueryable<Notification> AddOrder(
        this IQueryable<Notification> query,
        NotificationListOrder order,
        ...
        switch (order)
        {
            case NotificationListOrder.Created:
                query = descending
                    ...
                    : query.OrderBy(n => n.Created);
                break;

            case NotificationListOrder.StatusUpdated:
                query = descending
                    ? query.OrderByDescending(n => n.StatusUpdated)
                    : query.OrderBy(n => n.StatusUpdated);
                break;
        }
