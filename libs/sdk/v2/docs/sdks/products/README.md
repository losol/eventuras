...
async function run() {
  const result = await eventurasSDK.products.getV3ProductsProductIdSummary({
    productId: 653493,
  });
...
import { productsGetV3ProductsProductIdSummary } from "eventuras-sdk-v2/funcs/productsGetV3ProductsProductIdSummary.js";
...
async function run() {
  const res = await productsGetV3ProductsProductIdSummary(eventurasSDK, {
    productId: 653493,
  });
