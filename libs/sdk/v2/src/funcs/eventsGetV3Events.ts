 ...
 */
export async function eventsGetV3Events(
  client: EventurasSDKCore,
  request: operations.GetV3EventsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.EventDtoPageResponseDto,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetV3EventsRequest$outboundSchema.parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "IncludePastEvents": payload.IncludePastEvents,
    "Limit": payload.Limit,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.EventDtoPageResponseDto$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
