...
```json
{
  "mcpServers": {
    "EnrollmentsSDK": {
      "command": "npx",
      "args": ["-y", "--", "enrollments-sdk", "mcp", "start"],
      "env": {
        "ENROLLMENTSSDK_BEARER": "..."
      }
    }
  }
}
...
1. Create a shell script called `mcp-enrollments_sdk.sh` with the following content:

    ```sh
    #!/bin/sh
     
    export ENROLLMENTSSDK_BEARER="..."

    exec npx -y -- enrollments-sdk mcp start "$@"
    ```

2. Then make it executable with the following command:

    ```sh
    chmod +x mcp-enrollments_sdk.sh
    ```
...
3. In Cursor, `Cursor Settings > Features > MCP Servers > Add new MCP server` and use the following settings:
    ...
    | Name    | EnrollmentsSDK |
    | Type    | `command` |
    | Command | `/path/to/mcp-enrollments_sdk.sh` |
