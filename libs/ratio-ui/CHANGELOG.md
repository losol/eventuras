...
- 2842f02: Give `Container` a responsive default horizontal padding (`px-3 sm:px-4 lg:px-6`) so content stops running edge-to-edge on narrow viewports. Consumers that need a different value or none at all can override via `padding` or `paddingX` (any of those, including `paddingX="none"`, suppresses the default).
