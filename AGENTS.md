...
- **Run locally**: `dotnet run --project src/Eventuras.AppHost` (starts PostgreSQL, migrations, API, Keycloak, Mailpit, the web app and Traefik, reachable on `https://*.dev.localhost`)
...
- `src/Eventuras.AppHost` — Orchestrator that defines the application model (PostgreSQL, migrations, API, Keycloak, Mailpit, web, Traefik)
